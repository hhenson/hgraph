...
namespace hgraph
{
    ...
    struct HGRAPH_CLASS_EXPORT NodeTypeMetaData
    {
         ...
         * endpoints and pending scheduler events, or a sink with recordable state.
         * Scheduler recovery is independent of recordable state. Sources need operations: they hold cursors.
         */
        [[nodiscard]] bool checkpoints_without_ops() const noexcept
        {
            if (node_kind == NodeKind::Sink) { return recordable_state_schema != nullptr; }
            return node_kind == NodeKind::Compute && state_schema == nullptr &&
