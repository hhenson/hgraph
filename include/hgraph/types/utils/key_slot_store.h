...
namespace hgraph
{
    ...
    struct KeySlotStore
    {
      ...
      public:
        ...
        // ----- checkpoint rebuild (RFC 0023): chosen-slot import -----
        //
        // Slot ids are identity (parent child ids, checkpoint locators), and
        // the free-list's exact order encodes the erase history that decides
        // every future key→slot assignment. A restore therefore reconstructs
        // slots at their EXACT ids and then restores both slot lists
        // verbatim. The ordinary slot observers fire so representation-
        // internal mirrors (the TSD value store) reconstruct alongside; a
        // restore runs before any runtime subscriber exists.

        /** The free-list in its exact order (checkpoint capture surface). */
        [[nodiscard]] const std::vector<size_t> &checkpoint_free_slot_order() const noexcept {
            return m_free_slots;
        }

        /** Pending-erase slots in their exact order (checkpoint capture surface). */
        [[nodiscard]] const std::vector<size_t> &checkpoint_pending_erase_order() const noexcept {
            return m_pending_erase_slots;
        }

        /**
         * Reconstruct one slot at its exact id from a checkpoint image.
         * ``live == false`` rebuilds a pending-erase slot: constructed,
         * indexed, value mirror populated, awaiting the deferred flush —
         * exactly the state ``remove_slot`` leaves behind.
         */
        void checkpoint_import_slot(size_t slot, const ValueView &key, bool live) {
            require_value_binding();
            if (!key.has_value()) {
                throw std::invalid_argument("KeySlotStore checkpoint import requires a live key");
            }
            if (slot >= slot_capacity()) {
                throw std::logic_error("KeySlotStore checkpoint slot exceeds reserved capacity");
            }
            if (key_storage.constructed(slot)) {
                throw std::logic_error("KeySlotStore checkpoint slot is already constructed");
            }

            void *destination = key_storage.slot_memory(slot);
            m_value_binding.default_construct_at(destination);
            auto rollback_value =
                ::hgraph::make_scope_exit([&]() noexcept { m_value_binding.destroy_at(destination); });
            m_value_binding.ops_ref().copy_assign_from(
                m_value_binding, destination, key.binding(), key.data());
            key_storage.mark_staged(slot);
            rollback_value.release();
            ...
            if (live) {
                ...
                static_cast<void>(key_storage.mark_pending_erase(slot));
                m_pending_erase_slots.push_back(slot);
                ++m_pending_erase_count;
                observers.notify_remove(slot);
            }
        ...
        /** Restore the free-list and pending-erase orderings verbatim. */
        void checkpoint_set_slot_lists(std::vector<size_t> free_slots,
                                       std::vector<size_t> pending_erase_slots) {
            m_free_slots = std::move(free_slots);
            m_pending_erase_slots = std::move(pending_erase_slots);
            m_pending_erase_count = m_pending_erase_slots.size();
        }
