...
namespace hgraph
{
     ...
     * the unconstrained ``Any``, a Python class used as a type,
     * ``Frame[Row, Meta]`` and ``Series[T]``; ``TS``, ``TSS``, ``TSD``, ``TSL``, ``TSB``, ``TSW``
    ...
    class HGRAPH_CLASS_EXPORT SchemaTableReader
    {
      ...
      private:
        ...
        // Value entries that stand in for the writer's schema rather than
        // being it: a class used as a type, read where that class was never
        // registered, and anything built over one.
        ankerl::unordered_dense::set<std::size_t> substituted_{};
    };
}
