"""Ported from hgraph_unit_tests/adaptors/data_frame/test_data_source_generators.py.

Changes from upstream:
- Arrow-boundary ruling (2026-07-17): ``ts_of_frames_from_data_source`` emits
  ``pyarrow.Table`` frames; the two frame tests convert each tick with
  ``pl.from_arrow`` before the column-wise comparison. Everything else is
  upstream-verbatim.
"""
import polars as pl
...
def test_ts_of_frames_from_data_source():
    ...
    results = [[list(i) for i in pl.from_arrow(row)] for row in results]  # for i in row is columnar not row wise
    assert results == [
        [[1, 2], [4, 5]],
        [[3, 4], [6, 7]],
        [[5, 6], [8, 9]],
