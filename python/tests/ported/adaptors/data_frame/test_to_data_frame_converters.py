...
# pl.from_arrow(...) before the unchanged upstream assertions.
...
from hgraph.test import eval_node as _eval_node
import polars as pl


def eval_node(*args, **kwargs):
    """Arrow-boundary adaptation: convert recorded frames to polars."""
    result = _eval_node(*args, **kwargs)
    if result is None:
        return None
    return [pl.from_arrow(r) if r is not None and hasattr(r, "__arrow_c_stream__") else r
            for r in result]
