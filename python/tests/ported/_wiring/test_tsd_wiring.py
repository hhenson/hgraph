...
def test_tsd_in_bundle_ref():
    ...
    class AB_1(TimeSeriesSchema):
        a: TS[int]
        b: TSD[int, TSD[int, TS[int]]]

    @graph
    def source(i: TSS[int], v: TS[int]) -> TSB[AB_1]:
        m = map_(lambda key, x: map_(lambda y: y, key, __keys__=x), pass_through(i), __keys__=i)
        return switch_(
            v,
            {
                0: lambda i, m: combine[TSB[AB_1]](a=0, b=m),
                1: lambda i, m: combine[TSB[AB_1]](a=1, b=m),
            },
            i, m
        )
    ...
    # ``b`` has no news, so it is absent from the delta (TS-24, issue #835).
    # Released hgraph
