...
def test_convert_opaque_object_without_native_conversion_rejects():
    @graph
    def g(value: TS[object]) -> TS[int]:
        return convert[TS[int]](value)

    with pytest.raises(RuntimeError, match="no runtime value conversion.*py_object.*int"):
        eval_node(g, [base_test_class()])
