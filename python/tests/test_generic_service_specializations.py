...
def test_deferred_generic_duplicate_registrations_are_rejected():
    value_type = TypeVar("SERVICE_VALUE", int, str)

    @hg.reference_service
    def value(path: str = "shared") -> hg.TS[value_type]: ...

    @hg.service_impl(interfaces=value)
    def first(path: str, tp: Type[value_type] = hg.AUTO_RESOLVE):
        value[tp].wire_impl_out_stub(path, hg.const(1 if tp is int else "first"))

    @hg.service_impl(interfaces=value)
    def second(path: str, tp: Type[value_type] = hg.AUTO_RESOLVE):
        value[tp].wire_impl_out_stub(path, hg.const(2 if tp is int else "second"))

    @hg.graph
    def app() -> hg.TS[int]:
        hg.register_service("shared", first)
        hg.register_service("shared", second)
        return value[int]()

    with pytest.raises(
            ValueError,
            match="duplicate .* implementation candidate"):
        eval_node(app)
