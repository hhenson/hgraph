...
from .delta_adaptor import delta_query_adaptor
...
class DeltaQueryDataSource(DataSource):
    ...
    def render(self, **options) -> str:
        try:
            ...
            logger.exception(
                "Error rendering query for %s %s",
                self.__class__.__name__, self.source_path,
            )
...
def subscribe_delta_query_from_graph(
    ...
    return delta_query_adaptor[_schema].from_graph(
        ...
        tables=ds.tables,
        query=_render_query(ds, dce.scope, options),
        __request_id__=request_id,
    )


@subscriber_impl_to_graph
def subscribe_delta_query_to_graph(
    dce: DataCatalogueEntry,
    ds: TS[DeltaQueryDataSource],
    options: TS[dict[str, object]],
    request_id: TS[int],
