...
def as_array(tsw: TSW[SCALAR, WINDOW_SIZE, WINDOW_SIZE_MIN],
    ...
    if _sz.SIZE == _m_sz.SIZE:
        ...
        return _as_array_with_padding[SIZE: Size[_sz.SIZE]](tsw, hgraph.zero(TS[_tp], sum_) if zero is None else zero,
                                                            sz=_sz.SIZE)
...
@compute_node(all_valid=("tsw",))
def _as_array_with_padding(tsw: TSW[SCALAR, WINDOW_SIZE, WINDOW_SIZE_MIN], zero: TS[SCALAR], sz: int) -> TS[
    Array[SCALAR, SIZE]]:
    v = tsw.value
    if len(v) == sz:
        return v
