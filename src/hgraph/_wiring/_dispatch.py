...
def dispatch_(overloaded: BaseWiringNodeClass, *args, __on__: Tuple[str, ...] = None, **kwargs):
    ...
    """
    from hgraph import CustomMessageWiringError, switch_, TSL, SCALAR, with_signature, graph, nth, compute_node, TS, CompoundScalar
    from hgraph.nodes import flatten_tsl_values, downcast_ref

    if overloads := getattr(overloaded, 'overload_list', None):
        dispatch_args = {}
        if __on__ is None:
            for k, t in overloaded.signature.input_types.items():
                if t.is_scalar:
                    continue
            ...
            for k in __on__:
                t = overloaded.signature.input_types[k]
                if t.is_scalar:
        ...
        dispatch_map = {}
        for o, _ in overloads.overloads:
            ...
            def make_dispatch_graph(o, dispatch_types):
                @graph
                @with_signature(kwargs=overloaded.signature.input_types, return_annotation=overloaded.signature.output_type)
                def dispatch(**kwargs):
        ...
        kwargs = overloaded._prepare_kwargs(*args, **kwargs)  # process args and kwargs in kwargs so we can build a key
        ...
        if len(dispatch_args) == 1:
            ...
            key = adjust_dispatch_key(get_type(kwargs[nth(dispatch_args.keys(), 0)]), tuple(dispatch_map.keys()))
        else:
            @compute_node
            ...
            key = adjust_dispatch_keys(flatten_tsl_values[SCALAR: Type[CompoundScalar]](TSL.from_ts(*[get_type(kwargs[k]) for k in dispatch_args]), all_valid=True), tuple(dispatch_map.keys()))
