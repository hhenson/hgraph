    ...
    def impl_signature(self, __pre_resolved_types__: dict[TypeVar, HgTypeMetaData | Callable] = None):
        ...
        return self.signature.copy_with(
            input_types=frozendict(
                {k: HgTypeMetaData.parse_type(TSD[int, v.resolve(resolution_dict)]) for k, v in
                 self.signature.time_series_inputs.items()}
                | self.signature.scalar_inputs),
            output_type=HgTypeMetaData.parse_type(
                TSD[int, self.signature.output_type.resolve(
                    resolution_dict).py_type]) if self.signature.output_type else None
        )
