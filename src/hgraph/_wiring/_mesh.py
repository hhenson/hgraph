...
from hgraph._wiring._map import _deduce_signature_from_lambda_and_args, _extract_map_fn_key_arg_and_type, _split_inputs, \
    _MappingMarker, KEYS_ARG, _prepare_stub_inputs
...
def _build_mesh_wiring_node_and_inputs(
        fn: Callable, signature: WiringNodeSignature, *args, __keys__=None, __key_arg__=None, __name__=None,
    ...
    mesh_wiring_node = _create_mesh_wiring_node(fn, kwargs_, input_types, multiplex_args, no_key_args,
...
def _create_mesh_wiring_node(
    ...
    output_tsd_type = HgTSDTypeMetaData(input_key_tp.value_scalar_tp, HgREFTypeMetaData(
        resolved_signature.output_type.dereference()))
    ...
    output_type = HgTSBTypeMetaData(HgTypeMetaData.parse_type(
        UnNamedTimeSeriesSchema.create(
            out=output_tsd_type,
            ref=HgREFTypeMetaData(output_tsd_type))))
    ...
    provisional_signature = WiringNodeSignature(
        ...
        active_inputs=None,
        valid_inputs=frozenset({KEYS_ARG, }),
...
def get_mesh(fn_or_name: str | WiringNodeClass) -> MeshWiringPort | None:
    ...
    context = TimeSeriesContextTracker.instance().get_context(
        HgTypeMetaData.parse_type(TIME_SERIES_TYPE),
