...
from hgraph import compute_node, TS, TSB, TIME_SERIES_TYPE, str_, Match, match_, replace, split, OUT, DEFAULT, \
    HgTupleCollectionScalarType, join, TSL, SIZE, \
    format_, TS_SCHEMA, TS_SCHEMA_1, add_, graph, sub_, WiringError, mul_, STATE
    ...
    if out := mapping.get(OUT):
        if isinstance(out, HgTSLTypeMetaData):
            ...
            if isinstance(out_scalar, HgTupleFixedScalarType):
                ...
                return True if (e.matches_type(str) for e in out_scalar.element_types) \
                    else "Output type of split should be TSL[TS[str], Size]"
            elif isinstance(out_scalar, HgTupleCollectionScalarType):
                return True if out_scalar.element_type.matches_type(str) else "Output type of split should be TS[Tuple[str, ...]]"

    return f"Output type of {out} is not supported for split"


@compute_node(overloads=split, requires=_check_sizes,
              resolvers={OUT: lambda m, s: TS[Tuple[str, ...]]})
...
def format_(fmt: TS[str], *__pos_args__: TSB[TS_SCHEMA], __sample__: int = -1, _state: STATE[FormatState] = None,
            **__kw_args__: TSB[TS_SCHEMA_1]) -> TS[str]:
    if __sample__ > 1:
        _state.count += 1
