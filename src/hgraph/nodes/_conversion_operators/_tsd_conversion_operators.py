...
from hgraph import compute_node, OUT, TS, combine, TSL, SCALAR, SIZE, TIME_SERIES_TYPE, TSD, REF, convert, \
    KEYABLE_SCALAR, DEFAULT, TSD_OUT, REMOVE, collect, SIGNAL, TS_OUT, TSS, emit, V, STATE, SCHEDULER, TSB, MIN_TD
...
def convert_tuple_to_tsd(key: TS[Tuple[KEYABLE_SCALAR, ...]], ts: TS[Tuple[SCALAR, ...]], to: Type[OUT] = DEFAULT[OUT],
                         _output: TSD_OUT[KEYABLE_SCALAR, TIME_SERIES_TYPE] = None) -> TSD[KEYABLE_SCALAR, TIME_SERIES_TYPE]:
...
def convert_set_to_tsd(keys: TS[Set[KEYABLE_SCALAR]], ts: TIME_SERIES_TYPE, to: Type[OUT] = DEFAULT[OUT],
                       _output: TSD_OUT[KEYABLE_SCALAR, TIME_SERIES_TYPE] = None) -> TSD[KEYABLE_SCALAR, TIME_SERIES_TYPE]:
...
def convert_tss_to_tsd(keys: TSS[KEYABLE_SCALAR], ts: TIME_SERIES_TYPE, to: Type[OUT] = DEFAULT[OUT]
...
@compute_node(overloads=convert,
              requires=lambda m, s: m[OUT].py_type is TSD or
                                    m[OUT].matches_type(TSD[int, m[TIME_SERIES_TYPE].py_type]),
              )
...
def collect_tsd_from_tuples(keys: TS[Tuple[KEYABLE_SCALAR, ...]], ts: TS[Tuple[SCALAR, ...]], *, reset: SIGNAL = None, tp_: Type[OUT] = DEFAULT[OUT],
                _output: TSD_OUT[KEYABLE_SCALAR, TIME_SERIES_TYPE] = None) -> TSD[KEYABLE_SCALAR, TIME_SERIES_TYPE]:
    remove = {k: REMOVE for k in _output.keys()} if reset.modified else {}
...
def collect_tsd_from_mappings(ts: TS[Mapping[KEYABLE_SCALAR, SCALAR]], *, reset: SIGNAL = None,
...
@compute_node(overloads=emit,
              resolvers={OUT: lambda m, s: TS[m[TIME_SERIES_TYPE].scalar_type()]})
def emit_tsd(ts: TSD[KEYABLE_SCALAR, TIME_SERIES_TYPE],
