...
@compute_node(overloads=convert, requires=lambda m, s: m[SCALAR] != m[SCALAR_1] and _check_conversion_mode(0, m[SCALAR].py_type, m[SCALAR_1].py_type))
...
@graph(overloads=convert, requires=lambda m, s: m[SCALAR] != m[SCALAR_1] and _check_conversion_mode(1, m[SCALAR].py_type, m[SCALAR_1].py_type))
def convert_ts_scalar_upcast(ts: TS[SCALAR], to: type[TS[SCALAR_1]] = DEFAULT[OUT], s1_type: type[SCALAR_1] = AUTO_RESOLVE) -> TS[SCALAR_1]:
    return ts  # upcasting is implicit in the graph


@compute_node(overloads=convert, requires=lambda m, s: m[SCALAR] != m[SCALAR_1] and _check_conversion_mode(-1, m[SCALAR].py_type, m[SCALAR_1].py_type))
def convert_ts_scalar_downcast(ts: TS[SCALAR], to: type[TS[SCALAR_1]] = DEFAULT[OUT], s1_type: type[SCALAR_1] = AUTO_RESOLVE) -> TS[SCALAR_1]:
