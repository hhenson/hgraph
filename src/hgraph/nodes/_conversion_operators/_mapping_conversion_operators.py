...
from hgraph import compute_node, emit, TS, SCALAR, STATE, SCHEDULER, MIN_TD, convert, OUT, KEYABLE_SCALAR, \
    DEFAULT, TIME_SERIES_TYPE, collect, SIGNAL, TS_OUT, TSB, V, SIZE, TSL, TSD
...
def convert_tuples_to_mapping(key: TS[Tuple[KEYABLE_SCALAR, ...]], ts: TS[Tuple[SCALAR, ...]], to: Type[OUT] = DEFAULT[OUT]) -> TS[Mapping[KEYABLE_SCALAR, SCALAR]]:
...
def collect_mapping_from_tuples(key: TS[Tuple[KEYABLE_SCALAR, ...]], ts: TS[Tuple[SCALAR, ...]], *,
                                reset: SIGNAL = None, tp_: Type[OUT] = DEFAULT[OUT],
