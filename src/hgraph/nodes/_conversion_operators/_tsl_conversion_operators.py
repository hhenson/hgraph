...
from hgraph import graph, TSL, TIME_SERIES_TYPE, SIZE, combine, OUT, TIME_SERIES_TYPE_1, DEFAULT, convert, SCALAR, TS, \
    compute_node, emit, SCHEDULER, MIN_TD
...
@compute_node(overloads=convert,
              requires=lambda m, s: SIZE in m and (
                      m[OUT].py_type is TSL or m[OUT].value_tp.scalar_type().matches(m[SCALAR])),
              resolvers={TIME_SERIES_TYPE: lambda m, s: TS[m[SCALAR]] if m[OUT].py_type is TSL else m[OUT].value_tp,
                         SIZE: lambda m, s: None if m[OUT].py_type is TSL else m[OUT].size()},
              )
def convert_tuple_to_tsl(ts: TS[Tuple[SCALAR, ...]], to: Type[OUT] = DEFAULT[OUT], __strict__: bool = True) -> TSL[TIME_SERIES_TYPE, SIZE]:
    return ts.value
...
def convert_tuple_to_tsl(ts: TS[TUPLE], to: Type[OUT] = DEFAULT[OUT], __strict__: bool = True) -> TSL[TIME_SERIES_TYPE, SIZE]:
    return ts.value


@compute_node(overloads=emit,
              resolvers={SCALAR: lambda m, s: m[TIME_SERIES_TYPE].scalar_type()},)
def emit_tsl(ts: TSL[TIME_SERIES_TYPE, SIZE],
