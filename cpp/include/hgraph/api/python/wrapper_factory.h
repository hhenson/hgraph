...
namespace hgraph
{
    ...
    /**
     * Extract TSInputView from a PyTimeSeriesInput wrapper.
     * Returns an invalid TSInputView when object type does not match.
     */
    TSInputView unwrap_input_view(const nb::handle &obj);

    /**
     * Extract TSOutputView from a PyTimeSeriesOutput wrapper.
     * Returns an invalid TSOutputView when object type does not match.
     */
