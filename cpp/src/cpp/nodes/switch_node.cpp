...
namespace hgraph
{
    ...
    SwitchNode<K>::SwitchNode(int64_t node_ndx, std::vector<int64_t> owning_graph_id, NodeSignature::ptr signature,
                              ...
                              const std::unordered_map<std::string, int> &default_input_node_ids, int default_output_node_id)
    ...
    template <typename K> void SwitchNode<K>::eval() {
        ...
        if (key_ts->modified()) {
            ...
            if (reload_on_ticked_ || !active_key_.has_value() || !keys_equal(key_ts->value(), active_key_.value())) {
                if (active_key_.has_value()) {
                    ...
                    graph()->evaluation_engine()->add_before_evaluation_notification([graph_to_dispose, builder]() mutable {
                ...
                active_graph_ =
                    active_graph_builder_->make_instance(new_node_id, this->shared_from_this(), to_string(active_key_.value()));
        ...
        if (active_graph_ != nullptr) {
            ...
            if (graph_reset_ && output() != nullptr && !output()->modified()) { output()->invalidate(); }
            if (auto nec = dynamic_cast<NestedEngineEvaluationClock *>(active_graph_->evaluation_engine_clock().get())) {
                nec->reset_next_scheduled_evaluation_time();
            }
        }
    ...
    template <typename K> void SwitchNode<K>::wire_graph(graph_ptr &graph) {
        ...
        if (!has_specific) {
            if constexpr (std::is_same_v<K, nb::object>) { effective_key = nb::cast<K>(get_python_default()); }
        }
        ...
        if (input_ids_to_use) {
            for (const auto &[arg, node_ndx] : *input_ids_to_use) {
                ...
                if (arg == "key") {
                    ...
                    auto inner_ref = std::dynamic_pointer_cast<TimeSeriesReferenceInput>(inner_any);
                    auto outer_ref = std::dynamic_pointer_cast<TimeSeriesReferenceInput>(outer_any);
                    if (!inner_ref || !outer_ref) {
                        throw std::runtime_error(fmt::format("SwitchNode wire_graph expects REF inputs for arg '{}'", arg));
                    }
    ...
    template <typename K> void SwitchNode<K>::enumerate_nested_graphs(const std::function<void(graph_ptr)> &callback) const {
        if (active_graph_) { callback(active_graph_); }
    }
