...
namespace hgraph
{
    ...
    namespace
    {
        ...
        [[maybe_unused]] [[nodiscard]] bool dlpack_dtype_for(const TypeMeta *elem, nb::dlpack::dtype &out) noexcept {
            ...
            if (elem == scalar_type_meta<int32_t>()) {
                out = nb::dtype<int32_t>();
                return true;
            }
            if (elem == scalar_type_meta<int16_t>()) {
                out = nb::dtype<int16_t>();
                return true;
            }
            if (elem == scalar_type_meta<int8_t>()) {
                out = nb::dtype<int8_t>();
                return true;
            }
            if (elem == scalar_type_meta<uint64_t>()) {
                out = nb::dtype<uint64_t>();
                return true;
            }
            if (elem == scalar_type_meta<uint32_t>()) {
                out = nb::dtype<uint32_t>();
                return true;
            }
            if (elem == scalar_type_meta<uint16_t>()) {
                out = nb::dtype<uint16_t>();
                return true;
            }
            if (elem == scalar_type_meta<uint8_t>()) {
            ...
            if (elem == scalar_type_meta<float>()) {
                out = nb::dtype<float>();
                return true;
            }
        ...
        [[nodiscard]] void *resolve_element_ptr(View first, const TypeMeta *elem) {
            void *p = nullptr;
            if (elem == scalar_type_meta<int64_t>()) {
                p = first.try_as<int64_t>();
            } else if (elem == scalar_type_meta<double>()) {
                p = first.try_as<double>();
            } else if (elem == scalar_type_meta<float>()) {
                p = first.try_as<float>();
            } else if (elem == scalar_type_meta<int32_t>()) {
                p = first.try_as<int32_t>();
            } else if (elem == scalar_type_meta<int16_t>()) {
                p = first.try_as<int16_t>();
            } else if (elem == scalar_type_meta<int8_t>()) {
                p = first.try_as<int8_t>();
            } else if (elem == scalar_type_meta<uint64_t>()) {
                p = first.try_as<uint64_t>();
            } else if (elem == scalar_type_meta<uint32_t>()) {
                p = first.try_as<uint32_t>();
            } else if (elem == scalar_type_meta<uint16_t>()) {
                p = first.try_as<uint16_t>();
            } else if (elem == scalar_type_meta<uint8_t>()) {
                p = first.try_as<uint8_t>();
        ...
        [[nodiscard]] nb::object list_to_numpy(ListView &self) {
            ...
            if (typenum < 0) { throw std::runtime_error("List element type not buffer compatible for numpy"); }
            if (!ensure_numpy_c_api()) { throw std::runtime_error("Failed to initialise numpy C API"); }
            ...
            if (data == nullptr) { throw std::runtime_error("List element type not buffer compatible for numpy"); }
        ...
        [[nodiscard]] nb::object cyclic_buffer_to_numpy(const CyclicBufferView &self) {
            const TypeMeta *elem    = self.element_schema();
            const int       typenum = numpy_typenum_for(elem);
            if (typenum < 0) { throw std::runtime_error("CyclicBuffer element type not buffer compatible for numpy"); }
            if (!ensure_numpy_c_api()) { throw std::runtime_error("Failed to initialise numpy C API"); }
            ...
            if (n > 0) {
                auto *dst = static_cast<std::byte *>(PyArray_DATA(reinterpret_cast<PyArrayObject *>(arr)));
                for (size_t i = 0; i < n; ++i) {
                    View  element = self.at(i);
                    void *src     = resolve_element_ptr(element, elem);
                    if (src != nullptr) { std::memcpy(dst + i * elem_size, src, elem_size); }
                }
            }
            return nb::steal(arr);
        }
        ...
        void pycapsule_release_schema(PyObject *capsule) {
            ...
            if (schema != nullptr && schema->release != nullptr) { schema->release(schema); }
            delete schema;
        }

        void pycapsule_release_array(PyObject *capsule) {
            auto *array = static_cast<ArrowArray *>(PyCapsule_GetPointer(capsule, "arrow_array"));
            if (array != nullptr && array->release != nullptr) { array->release(array); }
            delete array;
        }
        ...
        struct KeySetView
        {
            ...
            [[nodiscard]] std::vector<View> values() const {
                ...
                for (const Value &key : m_keys) { result.push_back(key.view()); }
                return result;
            }
        ...
        struct PathElement
        {
            ...
            PathElement() = default;
            PathElement(const PathElement &other) : m_kind(other.m_kind), m_name(other.m_name), m_index(other.m_index) {
        ...
        [[nodiscard]] View navigate_impl(View current, const std::vector<PathElement> &path) {
            for (const PathElement &element : path) {
                ...
                if (element.is_field()) {
                    ...
                    } catch (const std::out_of_range &e) { throw std::runtime_error(e.what()); }
                    continue;
                }
                ...
                switch (schema->kind) {
                    case TypeKind::List: current = current.as_list().at(index); break;
                    case TypeKind::Tuple: current = current.as_tuple().at(index); break;
                    case TypeKind::Bundle: current = current.as_bundle().at(index); break;
                    case TypeKind::CyclicBuffer: current = current.as_cyclic_buffer().at(index); break;
                    case TypeKind::Map:
                        {
                            const TypeMeta *key_schema = current.as_map().key_schema();
                    ...
                    default: throw std::runtime_error("Index access is not supported for this view kind");
                }
            }
        ...
        [[nodiscard]] nb::object visit_python_value(View view, const nb::callable &fn) { return fn(view.to_python()); }

        void visit_python_value_void(View view, const nb::callable &fn) { static_cast<void>(fn(view.to_python())); }
        ...
        void deep_visit_impl(View current, PythonPath &path, const nb::callable &callback) {
            ...
            switch (schema->kind) {
                case TypeKind::Atomic: callback(current, python_path_list(path)); return;
                case TypeKind::Tuple:
                    {
                ...
                case TypeKind::Queue: callback(current, python_path_list(path)); return;
            }
        }
        ...
        void collect_leaf_paths_impl(View current, PythonPath &path, nb::list &result) {
            ...
            switch (schema->kind) {
                case TypeKind::Atomic: result.append(python_path_list(path)); return;
                case TypeKind::Tuple:
                    {
                        TupleView tuple = current.as_tuple();
                        for (size_t i = 0; i < tuple.size(); ++i) {
                ...
                case TypeKind::CyclicBuffer:
                case TypeKind::Queue: result.append(python_path_list(path)); return;
            }
        }
        ...
        [[nodiscard]] std::vector<PathElement> parse_path_impl(const std::string &path) {
            ...
            size_t i = 0;
            while (i < path.size()) {
                if (path[i] == '.') { throw std::runtime_error("Invalid path syntax"); }
        ...
        void register_path_support(nb::module_ &m) {
            nb::class_<PathElement>(m, "PathElement")
                .def_static(
                    "field", [](const std::string &name) { return PathElement::field(name); }, "name"_a)
                .def_static(
                    "index", [](size_t index) { return PathElement::index(index); }, "index"_a)
                .def_static(
                    "key", [](const View &key) { return PathElement::key(key); }, "key"_a)
            ...
            m.def(
                "navigate", [](View view, const std::vector<PathElement> &path) { return navigate_impl(view, path); }, "view"_a,
                "path"_a);
            m.def(
                "navigate", [](View view, const std::string &path) { return navigate_impl(view, parse_path_impl(path)); }, "view"_a,
                "path"_a);
            m.def(
                "try_navigate",
            ...
            m.def(
                "count_leaves", [](View view) { return count_leaves_impl(view); }, "view"_a,
                "Return the number of scalar leaves reachable from a nested value.");
            m.def(
                "collect_leaf_paths", [](View view) { return collect_leaf_paths_impl(view); }, "view"_a,
        ...
        void register_type_meta(nb::module_ &m) {
            nb::class_<TypeMeta>(m, "TypeMeta")
                ...
                .def_prop_ro(
                    "element_type", [](const TypeMeta &self) { return self.element_type; }, nb::rv_policy::reference)
                .def_prop_ro(
                    "key_type", [](const TypeMeta &self) { return self.key_type; }, nb::rv_policy::reference)
                .def_prop_ro(
                    "fields",
                    [](const TypeMeta &self) {
        ...
        void register_scalar_type_meta_functions(nb::module_ &m) {
            ...
            m.def("scalar_type_meta_timedelta", []() { return scalar_type_meta<engine_time_delta_t>(); }, nb::rv_policy::reference);
        }
        ...
        void register_view(nb::module_ &m) {
            nb::class_<View>(m, "_View")
                ...
                .def("is_scalar",
                     [](const View &self) { return self.schema() != nullptr && self.schema()->kind == TypeKind::Atomic; })
                .def("is_tuple",
                     [](const View &self) { return self.schema() != nullptr && self.schema()->kind == TypeKind::Tuple; })
                .def("is_bundle",
                     [](const View &self) { return self.schema() != nullptr && self.schema()->kind == TypeKind::Bundle; })
                ...
                .def("is_queue",
                     [](const View &self) { return self.schema() != nullptr && self.schema()->kind == TypeKind::Queue; })
                .def("to_string", &View::to_string)
                .def("to_python", &View::to_python)
                .def(
                    "from_python", [](View &self, nb::handle src) { self.from_python(nb::borrow<nb::object>(src)); }, "src"_a)
                .def("hash", &View::hash)
                .def("equals", &View::equals, "other"_a)
                .def("clone", [](const View &self) { return self.clone(); })
                .def("copy_from", &View::copy_from, "other"_a)
                .def(
                    "visit", [](View self, const nb::callable &fn) { return visit_python_value(self, fn); }, "fn"_a)
                .def(
                    "visit_void", [](View self, const nb::callable &fn) { visit_python_value_void(self, fn); }, "fn"_a)
                .def(
                    "visit_mut", [](View self, const nb::callable &fn) { visit_python_value_mut(self, fn); }, "fn"_a)
                ...
                .def(
                    "set_int", [](View &self, int64_t value) { self.as_atomic().set(value); }, "value"_a)
                .def(
                    "set_double", [](View &self, double value) { self.as_atomic().set(value); }, "value"_a)
                .def(
                    "set_bool", [](View &self, bool value) { self.as_atomic().set(value); }, "value"_a)
                .def(
                    "set_string", [](View &self, const std::string &value) { self.as_atomic().set(value); }, "value"_a)
                .def("try_as_int",
                ...
                .def(
                    "navigate", [](View self, const std::string &path) { return navigate_impl(self, parse_path_impl(path)); },
                    "path"_a)
                .def(
                    "navigate", [](View self, const std::vector<PathElement> &path) { return navigate_impl(self, path); }, "path"_a)
                .def(
                    "navigate_mut", [](View self, const std::string &path) { return navigate_impl(self, parse_path_impl(path)); },
                    "path"_a)
                .def(
                    "navigate_mut", [](View self, const std::vector<PathElement> &path) { return navigate_impl(self, path); },
                    "path"_a)
                .def(
                    "try_navigate",
        ...
        void register_tuple_view(nb::module_ &m) {
            nb::class_<TupleView, View>(m, "TupleView")
                ...
                .def(
                    "element_type", [](const TupleView &self, size_t index) { return self.at(index).schema(); }, "index"_a,
        ...
        void register_list_view(nb::module_ &m) {
            nb::class_<ListView, View>(m, "ListView")
                ...
                .def("to_numpy", [](ListView &self) -> nb::object { return list_to_numpy(self); })
                .def("__array__",
                     [](ListView &self, nb::kwargs kwargs) -> nb::object {
        ...
        void register_set_view(nb::module_ &m) {
            nb::class_<SetView, View>(m, "SetView")
                ...
                .def(
                    ...
                    "value"_a, "evaluation_time"_a = MIN_ST)
                .def(
                    "add",
                ...
                .def(
                    ...
                    "capacity"_a, "evaluation_time"_a = MIN_ST)
                .def(
                    "clear",
                    [](SetView &self, engine_time_t evaluation_time) {
                        auto mutation = self.begin_mutation(evaluation_time);
                        mutation.clear();
                    },
                    "evaluation_time"_a = MIN_ST);
        }
        ...
        void register_map_view(nb::module_ &m) {
            nb::class_<MapView, View>(m, "MapView")
                ...
                .def(
                    ...
                    "key"_a, "value"_a, "evaluation_time"_a = MIN_ST)
                .def(
                    "set",
                ...
                .def(
                    ...
                    "key"_a, "evaluation_time"_a = MIN_ST)
                .def(
                    "remove",
                ...
                .def(
                    "clear",
                    [](MapView &self, engine_time_t evaluation_time) {
                        auto mutation = self.begin_mutation(evaluation_time);
                        mutation.clear();
                    },
                    "evaluation_time"_a = MIN_ST)
        ...
        void register_sequence_views(nb::module_ &m) {
            nb::class_<CyclicBufferView, View>(m, "CyclicBufferView")
                ...
                .def("to_numpy", [](const CyclicBufferView &self) -> nb::object { return cyclic_buffer_to_numpy(self); })
                .def("__array__",
                     [](const CyclicBufferView &self, nb::kwargs kwargs) -> nb::object {
                         static_cast<void>(kwargs);
        ...
        void register_value(nb::module_ &m) {
            nb::class_<Value>(m, "Value")
                ...
                .def(
                    "__init__", [](Value *self, bool value) { new (self) Value(hgraph::value_for<bool>(std::move(value))); },
                    "value"_a)
                .def(
                    "__init__", [](Value *self, int64_t value) { new (self) Value(hgraph::value_for<int64_t>(std::move(value))); },
                    "value"_a)
                .def(
                    "__init__", [](Value *self, double value) { new (self) Value(hgraph::value_for<double>(std::move(value))); },
                    "value"_a)
                .def(
                    "__init__",
                ...
                .def(
                    "set_int", [](Value &self, int64_t value) { self.atomic_view().set(value); }, "value"_a)
                .def(
                    "set_double", [](Value &self, double value) { self.atomic_view().set(value); }, "value"_a)
                .def(
                    "set_bool", [](Value &self, bool value) { self.atomic_view().set(value); }, "value"_a)
                .def(
                    "set_string", [](Value &self, const std::string &value) { self.atomic_view().set(value); }, "value"_a)
                ...
                .def(
                    "from_python", [](Value &self, nb::handle src) { self.from_python(nb::borrow<nb::object>(src)); }, "src"_a)
                .def("hash", &Value::hash)
                ...
                .def(
                    ...
                    "path"_a)
                .def(
                    "navigate", [](Value &self, const std::vector<PathElement> &path) { return navigate_impl(self.view(), path); },
                    "path"_a)
                .def(
                    "navigate_mut",
                    [](Value &self, const std::string &path) { return navigate_impl(self.view(), parse_path_impl(path)); },
                ...
                .def(
                    "navigate_mut",
                    [](Value &self, const std::vector<PathElement> &path) { return navigate_impl(self.view(), path); }, "path"_a)
                .def(
                    "try_navigate",
