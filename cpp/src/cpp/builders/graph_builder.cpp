...
namespace hgraph
{
    ...
    graph_ptr GraphBuilder::make_instance(const std::vector<int64_t> &graph_id, node_ptr parent_node,
        ...
        size_t offset      = 0;
        auto   arena       = std::make_shared<char[]>(buffer_size);
        if (!arena) { throw std::bad_alloc(); }
        auto arena_buffer{std::static_pointer_cast<void>(arena)};

        // Construct the nodes, then the graph
        ...
        for (size_t i = 0; i < node_builders.size(); ++i) {
            ...
            nodes.push_back(node_builders[i]->make_instance(graph_id, i, arena_buffer, &offset));
        }
        ...
        for (const auto &edge : edges) {
            ...
            if (edge.output_path.size() == 1 && edge.output_path[0] == ERROR_PATH) { output = src_node->error_output(); } else if (
                edge.output_path.size() == 1 && edge.output_path[0] == STATE_PATH) {
                auto recordable_state = src_node->recordable_state();
                output                = std::dynamic_pointer_cast<TimeSeriesOutput>(recordable_state);
                if (!output) { throw std::runtime_error("recordable_state is not a TimeSeriesOutput"); }
            } else { output = edge.output_path.empty() ? src_node->output() : _extract_output(src_node, edge.output_path); }
        ...
        Graph::ptr graph_ptr = make_instance_impl<Graph, Graph>(arena_buffer, &offset, "Graph", graph_id, std::move(nodes),
                                                                parent_node, label,
