...
namespace hgraph
{
    node_ptr ComponentNodeBuilder::make_instance(const std::vector<int64_t> &owning_graph_id, int64_t node_ndx,
                                                 std::shared_ptr<void>       buffer, size_t *         offset) const {
        return _make_instance<ComponentNode>(buffer, offset, "ComponentNode", owning_graph_id, node_ndx, nested_graph_builder,
                                             input_node_ids,
                                             output_node_id);
    }
