...
namespace hgraph
{
    node_ptr ContextNodeBuilder::make_instance(const std::vector<int64_t> &owning_graph_id, int64_t node_ndx,
                                               std::shared_ptr<void>       buffer, size_t *         offset) const {
        return _make_instance<ContextStubSourceNode>(buffer, offset, "ContextStubSourceNode", owning_graph_id, node_ndx);
    }
