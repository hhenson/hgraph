...
namespace hgraph
{
    ...
    template <typename T> time_series_output_ptr TimeSeriesWindowOutputBuilder_T<T>::make_instance(node_ptr owning_node) const {
        return {new TimeSeriesFixedWindowOutput<T>(owning_node, size, min_size)};
    }

    template <typename T>
    time_series_output_ptr TimeSeriesWindowOutputBuilder_T<T>::make_instance(time_series_output_ptr owning_output) const {
        return {new TimeSeriesFixedWindowOutput<T>(dynamic_cast_ref<TimeSeriesType>(owning_output), size, min_size)};
    }
    ...
    template <typename T> time_series_output_ptr TimeSeriesTimeWindowOutputBuilder_T<T>::make_instance(node_ptr owning_node) const {
    ...
    template <typename T>
    time_series_output_ptr TimeSeriesTimeWindowOutputBuilder_T<T>::make_instance(time_series_output_ptr owning_output) const {
        auto v{nb::ref(new TimeSeriesTimeWindowOutput<T>(dynamic_cast_ref<TimeSeriesType>(owning_output), size, min_size))};
        return time_series_output_ptr{static_cast<TimeSeriesOutput *>(v.get())};
    }

    template <typename T> bool TimeSeriesTimeWindowOutputBuilder_T<T>::is_same_type(const Builder &other) const {
    ...
    template <typename T> void TimeSeriesTimeWindowOutputBuilder_T<T>::release_instance(time_series_output_ptr item) const {
        OutputBuilder::release_instance(item);
