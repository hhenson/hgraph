...
namespace hgraph {
    time_series_input_ptr TimeSeriesSignalInputBuilder::make_instance(const node_ptr& owning_node) const {
        auto v{new TimeSeriesSignalInput(owning_node)};
        return time_series_input_ptr{static_cast<TimeSeriesInput *>(v)};
    }

    time_series_input_ptr TimeSeriesSignalInputBuilder::make_instance(const time_series_input_ptr& owning_input) const {
