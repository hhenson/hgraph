...
namespace hgraph {
    template<typename T>
    time_series_input_ptr TimeSeriesValueInputBuilder<T>::make_instance(node_ptr owning_node, std::shared_ptr<void> buffer, size_t* offset) const {
        return make_instance_impl<TimeSeriesValueInput<T>, TimeSeriesInput>(
            buffer, offset, "TimeSeriesValueInput", owning_node);
    }

    template<typename T>
    time_series_input_ptr TimeSeriesValueInputBuilder<T>::make_instance(time_series_input_ptr owning_input, std::shared_ptr<void> buffer, size_t* offset) const {
        // Convert owning_input to TimeSeriesType shared_ptr
        auto owning_ts = std::dynamic_pointer_cast<TimeSeriesType>(owning_input);
        if (!owning_ts) {
            throw std::runtime_error("TimeSeriesValueInputBuilder: owning_input must be a TimeSeriesType");
        }
        return make_instance_impl<TimeSeriesValueInput<T>, TimeSeriesInput>(
            buffer, offset, "TimeSeriesValueInput", owning_ts);
    }
