...
namespace hgraph {
    ...
    TimeSeriesSetOutputBuilder::TimeSeriesSetOutputBuilder(const std::type_info& element_type)
    ...
    void TimeSeriesSetOutputBuilder::release_instance(time_series_output_ptr item) const {
        ...
        if (set == nullptr) {
            throw std::runtime_error("TimeSeriesSetOutputBuilder::release_instance: expected TimeSeriesSetOutput but got different type");
        }
        set->_reset();
    }
