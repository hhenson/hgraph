...
namespace hgraph
{
    template <> std::string to_string(const bool &value) { return value ? "true" : "false"; }

    template <> std::string to_string(const int64_t &value) { return std::to_string(value); }

    template <> std::string to_string(const double &value) { return std::to_string(value); }

    template <> std::string to_string(const engine_time_t &value) {
    ...
    template <> std::string to_string(const engine_date_t &value) {
        return std::format("{:04}-{:02}-{:02}", static_cast<int>(value.year()), static_cast<unsigned>(value.month()),
                           static_cast<unsigned>(value.day()));
    }

    template <> std::string to_string(const engine_time_delta_t &value) {
