...
namespace hgraph
{
    ...
    TimeSeriesReference::ptr TimeSeriesReference::make(time_series_output_ptr output) {
        if (output.get() == nullptr) { return make(); } else { return new BoundTimeSeriesReference(output); }
    }
            ...
            .def_static(
                ...
                [](nb::object ts, nb::object items) -> ptr {
                    if (!ts.is_none()) {
                        ...
                        if (nb::isinstance<TimeSeriesReferenceInput>(ts)) return nb::cast<TimeSeriesReferenceInput::ptr>(ts)->
                            value();
    ...
    void EmptyTimeSeriesReference::bind_input(TimeSeriesInput &ts_input) const {
        try { ts_input.un_bind_output(false); } catch (const std::exception &e) {
    ...
    void UnBoundTimeSeriesReference::bind_input(TimeSeriesInput &input_) const {
        ...
        for (size_t i = 0; i < _items.size(); ++i) {
            ...
            auto &r{_items[i]};
            if (r != nullptr) { r->bind_input(*item); } else if (item->bound()) { item->un_bind_output(false); }
        }
    ...
    void TimeSeriesReferenceOutput::copy_from_output(const TimeSeriesOutput &output) {
        ...
        if (output_t) { set_value(output_t->_value); } else {
    ...
    void TimeSeriesReferenceOutput::copy_from_input(const TimeSeriesInput &input) {
        ...
        if (input_t) { set_value(input_t->value()); } else {
    ...
    engine_time_t TimeSeriesReferenceInput::last_modified_time() const {
        ...
        if (_items.has_value()) { for (const auto &item : *_items) { times.push_back(item->last_modified_time()); } }
    ...
    void TimeSeriesReferenceInput::clone_binding(const TimeSeriesReferenceInput::ptr &other) {
        ...
        if (other->has_output()) { bind_output(other->output()); } else if (other->_items.has_value()) {
    ...
    void TimeSeriesReferenceInput::make_active() {
        if (has_output()) { TimeSeriesInput::make_active(); } else { set_active(true); }

        if (_items.has_value()) { for (auto &item : *_items) { item->make_active(); } }
    ...
    void TimeSeriesReferenceInput::make_passive() {
        if (has_output()) { TimeSeriesInput::make_passive(); } else { set_active(false); }

        if (_items.has_value()) { for (auto &item : *_items) { item->make_passive(); } }
    }
    ...
    bool TimeSeriesReferenceInput::do_bind_output(time_series_output_ptr &output_) {
        ...
        if (owning_node()->is_started()) {
            set_sample_time(owning_graph()->evaluation_clock()->evaluation_time());
            notify(sample_time());
        } else { owning_node()->add_start_input(this); }
        return false;
    }
    ...
    void TimeSeriesReferenceInput::register_with_nanobind(nb::module_ &m) {
        nb::class_<TimeSeriesReferenceInput, TimeSeriesInput>(m, "TimeSeriesReferenceInput")
            ...
            .def("__str__", [](const TimeSeriesReferenceInput &self) {
                ...
                if (self.has_value()) { value_str = self._value->to_string(); } else if (self.has_output()) {
                    value_str = "bound";
                } else if (self._items.has_value()) { value_str = fmt::format("{} items", self._items->size()); }
