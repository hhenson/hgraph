...
engine_time_t op_last_modified_time(const ViewData& vd) {
    ...
    const TSMeta* self_meta = meta_at_path(vd.meta, vd.path.indices);
    const bool include_wrapper_time =
        self_meta == nullptr || is_scalar_like_ts_kind(self_meta->kind) || self_meta->kind == TSKind::REF;
    const engine_time_t base_time =
        include_wrapper_time ? std::max(rebind_time, ref_wrapper_time) : rebind_time;
    ...
    if (self_meta != nullptr && self_meta->kind == TSKind::SIGNAL && vd.uses_link_target) {
        if (LinkTarget* signal_link = resolve_link_target(vd, vd.path.indices); signal_link != nullptr) {
            ...
            const engine_time_t current_eval_time = evaluation_time_for_view(vd);
    ...
    if (current != nullptr &&
        (current->kind == TSKind::TSB || (current->kind == TSKind::TSL && current->fixed_size() > 0))) {
...
bool op_modified(const ViewData& vd, engine_time_t current_time) {
    ...
    if (self_meta != nullptr && self_meta->kind == TSKind::REF) {
        ...
        if (vd.uses_link_target &&
            ...
            if (resolve_bound_target_view_data(vd, resolved_target) &&
                ...
                if (resolve_direct_bound_view_data(vd, direct_target) &&
                    ...
                    if (const TSMeta* direct_meta = meta_at_path(direct_target.meta, direct_target.path.indices);
                        direct_meta != nullptr && direct_meta->kind == TSKind::REF) {
                ...
                if (direct_target_is_ref) {
                    // Propagate only true source-wrapper rebinds. Wrapper-local
                    // same-identity writes are handled on the source wrapper but
                    // should not force downstream REF consumers to tick.
        ...
        if (static_ref_container) {
            ...
            if (has_unpeered_children && op_valid(vd)) {
        ...
        const bool type_erased_or_scalar_ref =
            element_meta == nullptr || is_scalar_like_ts_kind(element_meta->kind);
        ...
        if (resolve_read_view_data(vd, self_meta, resolved)) {
            ...
            if (value.has_value() && value->valid() && value->is_map()) {
                for_each_map_key_slot(value->as_map(), [&](View /*key*/, size_t slot) {
                    ...
                    child.path.indices.push_back(slot);
                    any_child_modified = op_modified(child, current_time);
                });
            }
...
bool op_valid(const ViewData& vd) {
    ...
    if (self_meta != nullptr && self_meta->kind == TSKind::REF) {
        ...
        const auto ref_wrapper_valid = [](const ViewData& ref_vd) -> bool {
            ...
            return op_last_modified_time(ref_vd) > MIN_DT;
        };
        ...
        if (debug_ref_valid) {
            ...
            std::fprintf(stderr,
                         ...
                         vd.path.to_string().c_str(),
                         static_cast<long long>(evaluation_time_for_view(vd).time_since_epoch().count()),
                         vd.uses_link_target ? 1 : 0,
                         local.has_value() ? 1 : 0,
                         (local.has_value() && local->valid()) ? 1 : 0,
                         static_cast<long long>(op_last_modified_time(vd).time_since_epoch().count()));
        }
        ...
        if (resolve_direct_bound_view_data(vd, direct_bound)) {
            const TSMeta* bound_meta = meta_at_path(direct_bound.meta, direct_bound.path.indices);
            const bool bound_valid =
                (bound_meta != nullptr && bound_meta->kind == TSKind::REF)
                    ? ref_wrapper_valid(direct_bound)
                    : op_valid(direct_bound);
            if (bound_valid) {
                if (debug_ref_valid) {
                    std::fprintf(stderr,
                                 "[ref_valid] path=%s source=direct_bound bound=%s kind=%d -> true\n",
                                 vd.path.to_string().c_str(),
    ...
    if (self_meta != nullptr && self_meta->kind == TSKind::REF && same_view_identity(resolved, vd)) {
        ...
        return op_last_modified_time(resolved) > MIN_DT;
    }
    const ViewData* data = &resolved;
...
bool op_all_valid(const ViewData& vd) {
    const TSMeta* self_meta = meta_at_path(vd.meta, vd.path.indices);
    if (self_meta != nullptr && self_meta->kind == TSKind::TSW) {
        ViewData resolved{};
        if (!resolve_read_view_data(vd, self_meta, resolved)) {
            return false;
        }
        ...
        if (current_value.has_value() && current_value->valid() && current_value->is_map()) {
                ...
                for (View key : set) {
                    ...
                    if (!in_added_set && has_added_set) {
                        ...
                        for (View added_key : added_keys.as_set()) {
                            ...
                            if (key_string.has_value()) {
                                try {
                                    if (added_key.to_string() == *key_string) {
                                        in_added_set = true;
                                        break;
                                    }
                                } catch (...) {}
                    ...
                    if (!key_visible_in_previous_view(key)) {
                        continue;
                    }
                    delta_out[key.to_python()] = remove;
                }
            ...
            if (!sampled_like && has_changed_map) {
                ...
                const auto key_in_added_set = [&added_keys](View key) {
                    ...
                    auto added_set = added_keys.as_set();
                    if (added_set.contains(key)) {
                        return true;
                    }
                    std::optional<std::string> key_string;
                    try {
                ...
                for (View key : changed_map.keys()) {
                    ...
                    if (nested_element) {
                        ...
                        const bool include_unmodified_ref_payload =
                            key_in_added_set(key) || !changed_entry_has_delta;
                        ...
                        nb::object child_delta;
                        if (include_unmodified_ref_payload) {
                        ...
                        if (child_meta != nullptr && child_meta->kind == TSKind::REF) {
                            nb::object child_delta_py =
                                ref_view_payload_to_python(child, child_meta, key_in_added_set(key));
                ...
                const auto key_in_removed_set = [&removed_keys](View key) {
                    ...
                    auto removed_set = removed_keys.as_set();
                    ...
                    for (View removed_key : removed_set) {
                        if (removed_key.equals(key)) {
                            return true;
                        }
                        if (key_string.has_value()) {
                            try {
                                if (removed_key.to_string() == *key_string) {
                                    return true;
                                }
                            } catch (...) {}
                        }
                ...
                const auto key_in_changed_map = [&changed_values](View key) {
                    ...
                    auto changed_map = changed_values.as_map();
                    if (changed_map.contains(key)) {
                        return true;
                    }
                    std::optional<std::string> key_string;
                    for (View changed_key : changed_map.keys()) {
                        if (changed_key.equals(key)) {
                            return true;
                        }
                        if (!key_string.has_value()) {
                            try {
                        ...
                        if (key_string.has_value()) {
                            try {
                                if (changed_key.to_string() == *key_string) {
                                    return true;
                                }
                            } catch (...) {}
                        }
                ...
                for_each_map_key_slot(value_map, [&](View key, size_t slot) {
                    ...
                    if (include_unmodified) {
                        ...
                        const TSMeta* child_meta = meta_at_path(child.meta, child.path.indices);
                        if (child_meta != nullptr && child_meta->kind == TSKind::REF) {
                            nb::object entry_py = ref_view_payload_to_python(child, child_meta, true);
                            if (entry_py.is_none()) {
                                return;
                            }
                    ...
                    const bool forced_from_structural_add = !include_unmodified && key_is_structural_add(key);
                    bool child_modified =
                    ...
                    if (!include_unmodified && !child_modified &&
                        child_meta != nullptr && child_meta->kind == TSKind::REF) {
                        ViewData target{};
                        if (resolve_bound_target_view_data(child, target)) {
                            child_modified = op_modified(target, current_time);
                        }
                    }
                    if (!include_unmodified && !child_modified) {
                        return;
                    ...
                    if (child_meta != nullptr && is_scalar_like_ts_kind(child_meta->kind)) {
                        ...
                        if (child_delta.is_none() && (forced_from_changed_map || forced_from_structural_add)) {
                            child_delta = op_to_python(child);
                        }
                        if (!child_delta.is_none()) {
                            delta_out[key.to_python()] = std::move(child_delta);
            ...
            if (!sampled_like && changed_values.valid() && changed_values.is_map()) {
                ...
                for (View key : changed_map.keys()) {
                    ...
                    if (child_meta != nullptr && child_meta->kind == TSKind::REF) {
                        entry = ref_view_payload_to_python(child, child_meta, true);
    ...
    if (current != nullptr && current->kind == TSKind::TSD) {
        ...
        for (const auto& kv : items) {
            ...
            if (is_remove || is_remove_if_exists) {
                ...
                View canonical_key = key;
                if (auto key_value = map_key_at_slot(dst_map, *removed_slot); key_value.has_value()) {
                    canonical_key = key_value->view();
                }
            ...
            if (scalar_like_element) {
                ...
                if (slot.has_value()) {
                    View canonical_key = key;
                    if (auto key_value = map_key_at_slot(dst_map, *slot); key_value.has_value()) {
                        canonical_key = key_value->view();
                    }
...
bool op_dict_remove(ViewData& vd, const View& key, engine_time_t current_time) {
    ...
    View canonical_key = key;
    if (auto key_value = map_key_at_slot(map, *removed_slot); key_value.has_value()) {
        canonical_key = key_value->view();
    }
    ViewData child_vd = vd;
    child_vd.path.indices.push_back(*removed_slot);
...
TSView op_dict_create(ViewData& vd, const View& key, engine_time_t current_time) {
    ...
    if (!existing_slot.has_value()) {
        ...
        View canonical_key = key;
        if (existing_slot.has_value()) {
            if (auto key_value = map_key_at_slot(map, *existing_slot); key_value.has_value()) {
                canonical_key = key_value->view();
            }
        }
...
TSView op_dict_set(ViewData& vd, const View& key, const View& value, engine_time_t current_time) {
    ...
    View canonical_key = key;
    if (auto key_value = map_key_at_slot(map, *slot); key_value.has_value()) {
        canonical_key = key_value->view();
    }
...
void copy_tsd(ViewData dst, const ViewData& src, engine_time_t current_time) {
    ...
    for (const auto& key : to_remove) {
        ...
        if (removed_slot.has_value()) {
            ...
            // copy_tsd() can drop ref-key visibility markers when a REF child
            // is non-empty but its resolved target has never become valid.
            // For key-set parity, treat those as non-visible removals.
            const TSMeta* child_meta = meta_at_path(child_vd.meta, child_vd.path.indices);
            if (child_meta != nullptr && child_meta->kind == TSKind::REF) {
                View child_value = op_value(child_vd);
                if (child_value.valid() && child_value.schema() == ts_reference_meta()) {
                    try {
                        TimeSeriesReference ref = nb::cast<TimeSeriesReference>(child_value.to_python());
                        removed_was_valid = ref.is_valid();
                    } catch (...) {}
                }
            }
        }
        ...
        } else if (!removed_was_valid && slots.added_set.valid() && slots.added_set.is_set()) {
    ...
    for (const auto& key : source_keys) {
        ...
        const View canonical_key = [&]() -> View {
            if (auto key_value = map_key_at_slot(dst_map, *slot); key_value.has_value()) {
                return key_value->view();
            }
            return key.view();
        }();
