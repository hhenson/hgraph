...
bool op_modified(const ViewData& vd, engine_time_t current_time) {
    ...
    const TSMeta* self_meta = meta_at_path(vd.meta, vd.path.indices);
    if (self_meta != nullptr && self_meta->kind == TSKind::TSValue) {
        const bool valid_now = op_valid(vd);

        // Scalar values read via REF/link-target paths should tick on valid
        // rebind edges (winner switches), even when the newly bound target has
        // no local modification this cycle.
        if (rebind_time_for_view(vd) == current_time && valid_now) {
            return true;
        }

        if (vd.uses_link_target) {
            // Suppress transitions where the resolved target is invalid/unbound.
            if (!valid_now) {
                return false;
            }
            ...
            if (resolve_bound_target_view_data(vd, bound_target) &&
                ...
                return op_last_modified_time(bound_target) == current_time;
            }
        }
    }
    ...
    if (self_meta != nullptr && self_meta->kind == TSKind::TSD) {
        const bool declared_ref_valued_tsd =
            self_meta->element_ts() != nullptr && self_meta->element_ts()->kind == TSKind::REF;
        ViewData resolved{};
        if (resolve_read_view_data(vd, self_meta, resolved)) {
            ...
            const bool ref_valued_tsd = element_meta != nullptr && element_meta->kind == TSKind::REF;
                    ...
                    if (suppress_ref_target_child_mods) {
                        ...
                        if (!any_child_modified && include_ref_target_child_mods) {
                            ...
                            if (child_meta != nullptr && child_meta->kind == TSKind::REF) {
                                ViewData target{};
                                any_child_modified =
                                    resolve_bound_target_view_data(child, target) && op_modified(target, current_time);
                            }
                        }
                    }
                });
    ...
    if (vd.uses_link_target) {
        // For REF wrappers, invalid rebind/unbind transitions should not be
        // forced into the generic "modified" path. Callers that need rebind
        // ticks already use explicit bridge/ref logic above.
        const bool skip_invalid_rebind_for_ref =
            self_meta != nullptr && self_meta->kind == TSKind::REF;
        if (skip_invalid_rebind_for_ref) {
            return false;
        }
        if (LinkTarget* link_target = resolve_link_target(vd, vd.path.indices);
            link_target != nullptr &&
...
bool op_valid(const ViewData& vd) {
    ...
    if (self_meta != nullptr && self_meta->kind == TSKind::TSValue && vd.uses_link_target) {
        if (LinkTarget* link_target = resolve_link_target(vd, vd.path.indices);
...
bool op_all_valid_tsl(const ViewData& vd) {
    const TSMeta* self_meta = meta_at_path(vd.meta, vd.path.indices);
    if (self_meta == nullptr || self_meta->kind != TSKind::TSL || self_meta->fixed_size() == 0) {
        return op_valid(vd);
    }
...
bool op_all_valid(const ViewData& vd) {
    ...
    if (self_meta->kind == TSKind::TSW) {
        return op_all_valid_tsw(vd);
    }
    if (self_meta->kind == TSKind::TSB) {
        return op_all_valid_tsb(vd);
    }
    if (self_meta->kind == TSKind::TSL) {
        return op_all_valid_tsl(vd);
    }
    return op_valid(vd);
}
...
View op_delta_value(const ViewData& vd) {
    ...
    if (self_meta != nullptr && self_meta->kind == TSKind::TSW) {
        return op_delta_value_tsw(vd);
    }
    if (self_meta != nullptr && is_scalar_like_ts_kind(self_meta->kind)) {
        return op_delta_value_scalar(vd);
    }
    return op_delta_value_container(vd);
}
...
void op_apply_delta(ViewData& vd, const View& delta, engine_time_t current_time) {
    ...
    if (current != nullptr && is_scalar_like_ts_kind(current->kind)) {
        op_apply_delta_scalar(vd, delta, current_time);
        return;
    }
