...
namespace hgraph {
    ...
    // ============================================================================
    // Tracking Schema Derivation
    // ============================================================================

    const value::TypeMeta* tracking_schema_from(const TSMeta* ts_meta) {
        if (!ts_meta) {
            return nullptr;
        }

        auto& registry = value::TypeRegistry::instance();
        const auto* time_type = value::scalar_type_meta<engine_time_t>();
        ...
        switch (ts_meta->kind()) {
            ...
            case TSTypeKind::REF:
                // Leaf types: just a single timestamp
                return time_type;

            case TSTypeKind::TSB: {
                // Bundle: create a bundle with tracking schema for each field
                const auto* bundle_meta = static_cast<const TSBTypeMeta*>(ts_meta);
                auto builder = registry.bundle();
                for (size_t i = 0; i < bundle_meta->field_count(); ++i) {
                    const auto& field = bundle_meta->field(i);
                    const auto* field_tracking = tracking_schema_from(field.type);
                    // Store the field name for the tracking bundle
                    const char* stored_name = registry.store_name(field.name);
                    builder.field(stored_name, field_tracking);
                }
                return builder.build();
            }

            case TSTypeKind::TSL: {
                // List: create a list with tracking schema for elements
                const auto* list_meta = static_cast<const TSLTypeMeta*>(ts_meta);
                const auto* elem_tracking = tracking_schema_from(list_meta->element_type());
                if (list_meta->is_fixed_size()) {
                    return registry.fixed_list(elem_tracking, list_meta->fixed_size()).build();
                } else {
                    return registry.list(elem_tracking).build();
                }
            }

            case TSTypeKind::TSD: {
                // Dict: map from key to timestamp (track modification per key)
                const auto* dict_meta = static_cast<const TSDTypeMeta*>(ts_meta);
                return registry.map(dict_meta->key_type(), time_type).build();
            }

            case TSTypeKind::TSS: {
                // Set: map from element to timestamp (track modification per element)
                const auto* set_meta = static_cast<const TSSTypeMeta*>(ts_meta);
                return registry.map(set_meta->element_type(), time_type).build();
            }
