...
namespace hgraph::v2
{
    namespace
    {
        [[nodiscard]] bool has_modified_active_input(Node &node, engine_time_t evaluation_time) {
            ...
            if (schema == nullptr || schema->kind != TSKind::TSB) { return node.input_view(evaluation_time).modified(); }
        ...
        void activate_active_inputs(Node &node, engine_time_t evaluation_time) {
            ...
            const auto input_slots = [&]() -> std::span<const size_t> {
                return (node.spec().has_explicit_active_inputs || !node.spec().active_inputs.empty()) ? node.spec().active_inputs
        ...
        [[nodiscard]] NodeErrorInfo fallback_node_error(const Node &node, std::string error_msg,
                                                        std::string additional_context = {}) {
            return NodeErrorInfo{
                node.runtime_label(), std::string{node.label()}, {}, std::move(error_msg), {}, {}, std::move(additional_context),
            };
        }
    ...
    Graph *detail::arg_provider<Graph *>::get(Node &node, engine_time_t) { return node.graph(); }
    ...
    TSInputView input_view_for(Node &node, engine_time_t evaluation_time) { return node.input_view(evaluation_time); }

    TSOutputView output_view_for(Node &node, engine_time_t evaluation_time) { return node.output_view(evaluation_time); }
    ...
    void NodeScheduler::schedule_immediate(std::optional<std::string> tag) {
        assert(m_node != nullptr);
        assert(m_node->graph() != nullptr);

        const std::string tag_value = tag.value_or("");
        if (tag.has_value()) {
            if (auto it = m_tags.find(tag_value); it != m_tags.end()) {
                m_scheduled_events.erase({it->second, tag_value});
            }
        }

        const engine_time_t now = m_node->graph()->evaluation_time();
        m_tags[tag_value]       = now;
        m_scheduled_events.insert({now, tag_value});

        if (m_node->started()) { m_node->graph()->schedule_node(m_node->node_index(), now); }
    }
    ...
    Node::Node(int64_t node_index, const BuiltNodeSpec *spec) noexcept
        : m_spec(spec), m_node_index(node_index), m_public_node_index(spec != nullptr && spec->public_node_index >= 0
                                                                          ? spec->public_node_index
                                                                          : node_index) {
    ...
    void Node::set_graph(Graph *graph) noexcept { m_graph = graph; }

    Graph *Node::graph() const noexcept { return m_graph; }

    const std::vector<int64_t> &Node::owning_graph_id() const noexcept {
        static const std::vector<int64_t> empty_graph_id;
        return m_graph != nullptr ? m_graph->graph_id() : empty_graph_id;
    }

    std::vector<int64_t> Node::node_id() const {
        std::vector<int64_t> out = owning_graph_id();
        out.push_back(public_node_index());
        return out;
    }

    int64_t Node::node_index() const noexcept { return m_node_index; }

    int64_t Node::public_node_index() const noexcept { return m_public_node_index; }

    std::string_view Node::label() const noexcept { return spec().label; }

    std::string Node::runtime_label() const { return spec().runtime_ops->runtime_label(*this); }

    NodeTypeEnum Node::node_type() const noexcept { return spec().node_type; }

    bool Node::is_push_source_node() const noexcept { return node_type() == NodeTypeEnum::PUSH_SOURCE_NODE; }

    bool Node::is_pull_source_node() const noexcept { return node_type() == NodeTypeEnum::PULL_SOURCE_NODE; }

    const TSMeta *Node::input_schema() const noexcept { return spec().input_schema; }

    const TSMeta *Node::output_schema() const noexcept { return spec().output_schema; }

    const TSMeta *Node::error_output_schema() const noexcept { return spec().error_output_schema; }

    const TSMeta *Node::recordable_state_schema() const noexcept { return spec().recordable_state_schema; }

    bool Node::has_input() const noexcept { return spec().runtime_ops->has_input(*this); }

    bool Node::has_output() const noexcept { return spec().runtime_ops->has_output(*this); }

    bool Node::has_error_output() const noexcept { return spec().runtime_ops->has_error_output(*this); }

    bool Node::has_recordable_state() const noexcept { return spec().runtime_ops->has_recordable_state(*this); }

    bool Node::started() const noexcept { return m_started; }

    bool Node::uses_scheduler() const noexcept { return spec().uses_scheduler; }

    bool Node::has_scheduler() const noexcept { return uses_scheduler(); }

    engine_time_t Node::evaluation_time() const noexcept { return m_graph != nullptr ? m_graph->evaluation_time() : MIN_DT; }

    void Node::set_started(bool value) noexcept { m_started = value; }
    ...
    void *Node::data() noexcept { return reinterpret_cast<std::byte *>(this) + spec().runtime_data_offset; }

    const void *Node::data() const noexcept { return reinterpret_cast<const std::byte *>(this) + spec().runtime_data_offset; }

    TSInputView Node::input_view(engine_time_t evaluation_time) { return spec().runtime_ops->input_view(*this, evaluation_time); }
