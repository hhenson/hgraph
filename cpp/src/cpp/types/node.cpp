...
namespace hgraph
{
    ...
    void NodeSignature::register_with_nanobind(nb::module_ &m) {
        nb::class_<NodeSignature, intrusive_base>(m, "NodeSignature")
            .def("__init__",
                 [](NodeSignature *self, nb::kwargs kwargs) {
                     new(self) NodeSignature(
                         nb::cast<std::string>(kwargs["name"]),
                         nb::cast<NodeTypeEnum>(kwargs["node_type"]),
                         nb::cast<std::vector<std::string>>(kwargs["args"]),
                         kwargs.contains("time_series_inputs")
                             ? nb::cast<std::optional<std::unordered_map<
                                 std::string, nb::object>>>(
                                 kwargs["time_series_inputs"])
                             : std::nullopt,
                         kwargs.contains("time_series_output")
                             ? nb::cast<std::optional<nb::object>>(
                                 kwargs["time_series_output"])
                             : std::nullopt,
                         kwargs.contains("scalars")
                             ? nb::cast<std::optional<nb::dict>>(
                                 kwargs["scalars"])
                             : std::nullopt,
                         kwargs["src_location"],
                         kwargs.contains("active_inputs")
                             ? nb::cast<std::optional<std::unordered_set<
                                 std::string>>>(kwargs["active_inputs"])
                             : std::nullopt,
                         kwargs.contains("valid_inputs")
                             ? nb::cast<std::optional<std::unordered_set<
                                 std::string>>>(kwargs["valid_inputs"])
                             : std::nullopt,
                         kwargs.contains("all_valid_inputs")
                             ? nb::cast<std::optional<std::unordered_set<
                                 std::string>>>(kwargs["all_valid_inputs"])
                             : std::nullopt,
                         kwargs.contains("context_inputs")
                             ? nb::cast<std::optional<std::unordered_set<
                                 std::string>>>(kwargs["context_inputs"])
                             : std::nullopt,
                         kwargs.contains("injectable_inputs")
                             ? nb::cast<std::optional<std::unordered_map<
                                 std::string, InjectableTypesEnum>>>(
                                 kwargs["injectable_inputs"])
                             : std::nullopt,
                         nb::cast<size_t>(kwargs["injectables"]),
            ...
            .def_prop_ro("uses_output_feedback",
                         &NodeSignature::uses_output_feedback)
            .def_prop_ro("uses_recordable_state",
                         &NodeSignature::uses_recordable_state)
            .def_prop_ro("test_recordable_property",
                         [](const NodeSignature &self) -> bool { return self.uses_recordable_state(); })
            ...
            .def_prop_ro("is_push_source_node",
                         &NodeSignature::is_push_source_node)
            .def_prop_ro("is_pull_source_node",
                         &NodeSignature::is_pull_source_node)
            ...
            .def("__repr__", [](const NodeSignature &self) {
                ...
                if (self.time_series_inputs.has_value()) {
                    oss << "{...}";
                    // dict with " << self.time_series_inputs->size() << " items
                } else { oss << "None"; }

                // time_series_output
                oss << ", time_series_output=";
                if (self.time_series_output.has_value()) { oss << "<object>"; } else { oss << "None"; }

                // scalars
                oss << ", scalars=";
                if (self.scalars.has_value()) { oss << "{...}"; } else { oss << "None"; }
                ...
                // capture_exception
                oss << ", capture_exception=" << (self.capture_exception
                                                      ? "True"
                                                      : "False");
                ...
                // label
                oss << ", label=";
                if (self.label.has_value()) { oss << "'" << *self.label << "'"; } else { oss << "None"; }

                // capture_values
                oss << ", capture_values=" << (self.capture_values
                                                   ? "True"
                                                   : "False");

                // record_replay_id
                oss << ", record_replay_id=";
                if (self.record_replay_id.has_value()) { oss << "'" << *self.record_replay_id << "'"; } else { oss << "None"; }
    ...
    void NodeScheduler::un_schedule() { if (!_scheduled_events.empty()) { _scheduled_events.erase(_scheduled_events.begin()); } }
    ...
    void NodeScheduler::register_with_nanobind(nb::module_ &m) {
        nb::class_<NodeScheduler, intrusive_base>(m, "NodeScheduler")
            .def_prop_ro("next_scheduled_time",
                         &NodeScheduler::next_scheduled_time)
            ...
            .def(
                "pop_tag",
                [](NodeScheduler &self, const std::string &tag,
                   engine_time_t  default_time) {
                    return self.pop_tag(tag, default_time);
                },
                "tag"_a, "default_time"_a)
            .def(
                "schedule",
                [](NodeScheduler &            self, engine_time_t when,
                   std::optional<std::string> tag, bool           on_wall_clock) {
                    self.schedule(when, std::move(tag), on_wall_clock);
                },
                "when"_a, "tag"_a = nb::none(), "on_wall_clock"_a = false)
            .def(
                "schedule",
                [](NodeScheduler &            self, engine_time_delta_t when,
                   std::optional<std::string> tag, bool                 on_wall_clock) {
                    self.schedule(when, std::move(tag), on_wall_clock);
                },
                "when"_a, "tag"_a = nb::none(), "on_wall_clock"_a = false)
            .def("un_schedule",
                 static_cast<void (NodeScheduler::*)(const std::string &)>(&
                     NodeScheduler::un_schedule), "tag"_a)
            .def("un_schedule",
                 static_cast<void (NodeScheduler::*)()>(&
                     NodeScheduler::un_schedule))
    ...
    void Node::notify(engine_time_t modified_time) {
        ...
        } else { scheduler()->schedule(MIN_ST, "start"); }
    }
    ...
    void Node::register_with_nanobind(nb::module_ &m) {
        nb::class_<Node, ComponentLifeCycle>(m, "Node")
            ...
            .def_prop_rw(
                "graph", static_cast<graph_ptr (Node::*)() const>(&Node::graph),
                &Node::set_graph)
            .def_prop_rw(
                "input",
                static_cast<time_series_bundle_input_ptr (Node::*)() const>(&
                    Node::input), &Node::set_input)
            .def_prop_ro("inputs",
                         [](Node &self) {
                             nb::dict d;
            ...
            .def_prop_rw("recordable_state", &Node::recordable_state,
                         &Node::set_recordable_state)
            ...
            .def_prop_rw("error_output", &Node::error_output,
                         &Node::set_error_output)
    ...
    std::string Node::repr() const {
        ...
        if (!_owning_graph_id.empty()) { graph_id_str = fmt::format("{}, {}", graph_id_str, _node_ndx); } else {
    ...
    void Node::eval() {
        ...
        if (should_eval) {
            try { do_eval(); } catch (const NodeException &e) {
                ...
                if (signature().capture_exception && error_output().get() != nullptr) {
                    ...
                    auto error_ptr = nb::ref<NodeError>(new NodeError(ne));
                    try { error_output()->py_set_value(nb::cast(error_ptr)); } catch (const std::exception &set_err) {
                        error_output()->py_set_value(nb::str(ne.to_string().c_str()));
                    } catch (...) { error_output()->py_set_value(nb::none()); }
                    return; // swallow after routing
                } else { throw NodeException::capture_error(e, *this, "During evaluation"); }
            } catch (...) {
                if (signature().capture_exception && error_output().get() != nullptr) {
