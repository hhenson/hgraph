...
namespace hgraph
{
    ...
    void Graph::evaluate_graph()
    {
        ...
        for (size_t i = push_source_nodes_end(); i < nodes.size(); ++i)
        {
            ...
            if (scheduled_time == now)
            {
                ...
                catch (const NodeException& e) { throw e; }
                catch (const std::exception& e)
                {
    ...
    void Graph::register_with_nanobind(nb::module_& m)
    {
        nb::class_ < Graph, ComponentLifeCycle > (m, "Graph")
                                                 .def(nb::init<std::vector<int64_t>, std::vector<node_ptr>,
                                                               std::optional<node_ptr>, std::string, traits_ptr>(),
                                                 ...
                                                 .def_prop_ro("evaluation_clock",
                                                              static_cast<EvaluationClock::ptr (Graph::*)() const>(&
                                                                  Graph::evaluation_clock))
                                                 .def_prop_ro("engine_evaluation_clock",
                                                              &Graph::evaluation_engine_clock)
                                                 .def_prop_rw("evaluation_engine", &Graph::evaluation_engine,
                                                              &Graph::set_evaluation_engine)
                                                 .def_prop_ro("push_source_nodes_end", &Graph::push_source_nodes_end)
                                                 .def("schedule_node",
                                                      static_cast<void (Graph::*)(int64_t, engine_time_t, bool)>(&
                                                          Graph::schedule_node), "node_ndx"_a,
