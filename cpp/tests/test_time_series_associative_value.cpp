...
namespace
{
    ...
    hgraph::engine_time_t next_test_time()
    {
        static size_t tick = 0;
        return hgraph::MIN_ST + hgraph::MIN_TD * tick++;
    }
}  // namespace
...
TEST_CASE("Set values support add contains and remove")
{
    ...
    {
        auto mutation = set.begin_mutation(next_test_time());
...
TEST_CASE("Set values retain removed payloads by slot until reuse")
{
    ...
    auto mutation_set = set.begin_mutation(next_test_time());
...
TEST_CASE("Map values support lookup and require live values")
{
    ...
    {
        auto mutation = map.begin_mutation(next_test_time());
...
TEST_CASE("Map values retain removed key and value payloads by slot until reuse")
{
    ...
    auto mutation_map = map.begin_mutation(next_test_time());
    CHECK_FALSE(delta.slot_occupied(removed_slot));
}
...
TEST_CASE("Associative mutation scopes support nesting with depth tracking")
{
    ...
    auto one = hgraph::value_for(int32_t{1});
    {
        const auto set_tick = next_test_time();
        auto outer = set.begin_mutation(set_tick);
        CHECK(outer.add(one.view()));
        CHECK(outer.remove(one.view()));
        {
            auto inner = set.begin_mutation(set_tick);
            CHECK_FALSE(set_delta.slot_occupied(0));
        }
        CHECK_FALSE(set_delta.slot_occupied(0));
    }
    {
        auto reopen = set.begin_mutation(next_test_time());
        CHECK_FALSE(set_delta.slot_occupied(0));
    ...
    {
        const auto map_tick = next_test_time();
        auto outer = map.begin_mutation(map_tick);
        outer.set(key.view(), value.view());
        CHECK(outer.remove(key.view()));
        {
            auto inner = map.begin_mutation(map_tick);
            CHECK_FALSE(map_delta.slot_occupied(0));
        }
        CHECK_FALSE(map_delta.slot_occupied(0));
    }
    {
        auto reopen = map.begin_mutation(next_test_time());
        CHECK_FALSE(map_delta.slot_occupied(0));
    ...
    existing_set.begin_mutation(next_test_time()).adding(int32_t{2});
    {
        const auto existing_set_tick = next_test_time();
        auto outer = existing_set.begin_mutation(existing_set_tick);
        CHECK(outer.remove(hgraph::value_for(int32_t{2}).view()));
        {
            auto inner = existing_set.begin_mutation(existing_set_tick);
    ...
    existing_map.begin_mutation(next_test_time()).setting(int32_t{9}, int32_t{90});
    {
        const auto existing_map_tick = next_test_time();
        auto outer = existing_map.begin_mutation(existing_map_tick);
        CHECK(outer.remove(hgraph::value_for(int32_t{9}).view()));
        {
            auto inner = existing_map.begin_mutation(existing_map_tick);
...
TEST_CASE("Associative delta views expose net added and removed slots")
{
    ...
    auto set = set_value.set_view();
    auto set_delta = set.delta();
    set.begin_mutation(next_test_time()).adding(int32_t{1}).adding(int32_t{2}).removing(int32_t{1});
    ...
    auto map = map_value.map_view();
    auto map_delta = map.delta();
    map.begin_mutation(next_test_time()).setting(int32_t{7}, int32_t{70}).setting(int32_t{8}, int32_t{80}).removing(int32_t{7});
...
TEST_CASE("Map delta views expose updated items for existing keys")
{
    ...
    map.begin_mutation(next_test_time()).setting(int32_t{7}, int32_t{70});
    map.begin_mutation(next_test_time()).setting(int32_t{7}, int32_t{700});
...
TEST_CASE("Associative mutation scopes support fluent command-style chaining")
{
    ...
    auto one = hgraph::value_for(int32_t{1});
    auto two = hgraph::value_for(int32_t{2});
    set.begin_mutation(next_test_time()).adding(one.view()).adding(two.view()).removing(one.view());
    CHECK_FALSE(set.contains(one.view()));
    CHECK(set.contains(two.view()));
    ...
    map.begin_mutation(next_test_time())
        .setting(key_a.view(), val_a.view())
        .setting(key_b.view(), val_b.view())
        .removing(key_a.view());
...
TEST_CASE("Associative mutation scopes accept native C++ values directly")
{
    ...
    auto map = map_value.map_view();
    map.begin_mutation(next_test_time())
        .setting(int32_t{7}, std::string{"seven"})
        .setting(int32_t{8}, std::string{"eight"})
        .removing(int32_t{7});
    CHECK_FALSE(map.contains(hgraph::value_for(int32_t{7}).view()));
