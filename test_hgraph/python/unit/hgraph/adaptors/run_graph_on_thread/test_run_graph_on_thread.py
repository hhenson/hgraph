...
from hgraph import MIN_ST, TS, EvaluationMode, GlobalState, GraphConfiguration, evaluate_graph, get_recorded_value, graph, record, register_adaptor
...
def test_run_graph_on_thread():
    ...
    def main():
        ...
        record(run_graph_on_thread[TS[int]](
            fn=simulation,
            global_state={'eh': 0},
            params=dict(
                a=1, 
    ...
    with GlobalState():
        evaluate_graph(main, config=GraphConfiguration(run_mode=EvaluationMode.REAL_TIME, end_time=timedelta(seconds=10)))
