...
# List TypeOps now implemented
def test_fixed_list_clear_invalidates_entries(fixed_double_list_schema):
    """Fixed-size list clear() preserves shape and invalidates entries."""
    v = Value(fixed_double_list_schema)

    v.reset()
    flv = v.list_view()
    flv.set(0, make_double_value(1.5).view())
    flv.set(1, make_double_value(2.5).view())

    flv.clear()

    assert len(flv) == fixed_double_list_schema.fixed_size
    assert not flv[0].has_value()
    assert not flv[1].has_value()
...
def test_list_view_on_list_value(dynamic_int_list_schema):
    """list_view() returns view for list values once the kind check passes."""
    v = Value(dynamic_int_list_schema)

    v.reset()
    assert v.view().is_list()
    result = v.view().as_list()
    assert result is not None


def test_list_view_on_non_list_value():
    """list_view() throws for non-list values."""
    v = Value(42)
    assert not v.view().is_list()
...
def test_const_list_view_read_only(dynamic_int_list_schema):
    """View.list_view() provides read access."""
    v = Value(dynamic_int_list_schema)
