...
from hgraph import TS, compute_node, const, generator, graph, null_sink, sink_node, wire_graph
...
def _clear_cpp_mapping_registry():
    original_mappings = get_cpp_node_mappings()
    clear_cpp_node_mappings()
    yield
    set_cpp_node_mappings(original_mappings)
...
def test_cpp_mapping_dispatch_uses_registered_builder_for_sink(monkeypatch):
    from hgraph._wiring._wiring_node_class import _python_wiring_node_classes as _pwc

    monkeypatch.setattr(_pwc, "_is_cpp_runtime_enabled_for_dispatch", lambda: True)

    @sink_node
    def mapped_sink(ts: TS[int]):
        _ = ts.value
    ...
    register_cpp_node_builder_for_callable(mapped_sink.fn, _mapped_builder)
