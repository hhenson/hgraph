...
| Parameter packs | partial | Implemented through syntax, typed HIR, hgraph IR, descriptor format v2, call binding, composition and runtime traversal, generated `VarIn`/`VarKwIn` contracts, and native runtime-node aggregate inputs. `...T` is homogeneous positional and lowers to `Args<>`/TSL; `<...Ts>(x: ...Ts)` is heterogeneous positional/tuple-shaped and `<...Fields>(x: ...{Fields})` is heterogeneous named/bundle-shaped, both lowered to `Kwargs<>`/TSB with explicit call-style registration. The accepted `{min:max}` cardinality and `len`/`keys`/`types`/`type_at`/`each` reflection semantics remain implementation work ([ADR 0007](decisions/0007-parameter-packs.md)). |
