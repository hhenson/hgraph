...
| Parameter packs | partial | Implemented through syntax, typed HIR, hgraph IR, descriptor format v2, call binding, composition traversal, and generated `VarIn`/`VarKwIn` contracts. `...T` is homogeneous positional, `<...Ts>(x: ...Ts)` heterogeneous positional/tuple-shaped, and `<...Fields>(x: ...{Fields})` heterogeneous named/bundle-shaped. Runtime nodes lower to the existing `Args<>`/TSL and `Kwargs<>`/TSB inputs; compiler emission remains to be connected. The accepted `{min:max}` cardinality and `len`/`keys`/`types`/`type_at`/`each` reflection semantics remain implementation work ([ADR 0007](decisions/0007-parameter-packs.md)). |
