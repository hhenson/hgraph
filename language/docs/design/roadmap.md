...
| Recursive struct fields ([ADR 0012](decisions/0012-recursive-struct-fields.md)) | partial | The resolver admits an optional `atomic<T>` edge and names the rule any other edge breaks; typed HIR and hgraph IR mark edges by identity; both backends realize an edge as an owner of its target through hgraph's `recursive_bundle_closure` (hgraph RFC 0041), which registers each group of mutually reachable specializations as one `recursive_bundles` batch, and give the temporal shape a `TS[Owned[T]]` endpoint that binds as `TS[T]`; generated C++ spells an edge `hgraph::Edge<T>` and agrees with direct wiring tick for tick. Fail closed: exporting a struct with an edge from `emit-cpp`, since module descriptors do not record edges; recursion through a container, a generic argument, or inheritance. A value of an abstract edge's family cannot yet be built in HGL, since a concrete struct is not assignable to its abstract parent. |
