...
| Recursive struct fields ([ADR 0012](decisions/0012-recursive-struct-fields.md)) | partial | The resolver admits an optional `atomic<T>` edge and names the rule any other edge breaks; typed HIR and hgraph IR mark edges by identity; direct wiring realizes an edge as an owner of its target, registers each group of mutually reachable specializations as one `recursive_bundles` batch, and gives the temporal shape a `TS[Owned[T]]` endpoint that binds as `TS[T]`. Fail closed: `emit-cpp` ("recursive edge ... is not supported by emit-cpp yet") until the static schema can spell an edge; module descriptors do not record edges; recursion through a container, a generic argument, or inheritance. A value of an abstract edge's family cannot yet be built in HGL, since a concrete struct is not assignable to its abstract parent. |
