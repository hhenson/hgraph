...
| Parameter packs | partial | Implemented through syntax, typed HIR, hgraph IR, descriptor format v2, call binding, composition traversal, and generated `VarIn`/`VarKwIn` contracts. `...T` is homogeneous positional, `<...Ts>(x: ...Ts)` heterogeneous positional/tuple-shaped, and `<...Fields>(x: ...{Fields})` heterogeneous named/bundle-shaped. Runtime-node aggregate views and type-pack reflection in `requires` remain fail-closed ([ADR 0007](decisions/0007-parameter-packs.md)). |
