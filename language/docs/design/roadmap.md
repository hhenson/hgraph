...
| Parameter packs | partial | Implemented through syntax, typed HIR, hgraph IR, descriptor format v3, call binding, composition and runtime traversal, generated `VarIn`/`VarKwIn` contracts, native runtime-node aggregate inputs, `{n}`/`{n:*}`/`{n:m}` cardinality, and the compile-time `len`/`keys`/`types`/`type_at` reflection intrinsics. `...T` is homogeneous positional and lowers to `Args<>`/TSL; `<...Ts>(x: ...Ts)` is heterogeneous positional/tuple-shaped and `<...Fields>(x: ...{Fields})` is heterogeneous named/bundle-shaped, both lowered to `Kwargs<>`/TSB with explicit call-style registration. The accepted `each` conjunction and runtime `schemas(values)` view remain implementation work ([ADR 0007](decisions/0007-parameter-packs.md)). |
