...
| MIG-005 recordable state | partial | B2, B4 | [ADR 0008](decisions/0008-temporal-contracts-and-target-mappings.md) settles cache vs state | Generic state without a default; sparse state; queues and windows; non-scalar caches and HGL mixed recordable-state/cache lowering; wall-clock schedule recovery; owned native state construction. Scalar caches, native pending alarms, and finite schedule progress in simulation are implemented. |
