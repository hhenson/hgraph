...
| MIG-012 effects and capabilities | partial | B1, B2, B5 | Descriptor phase/effect/ownership metadata ([native interface](native-interface.md#descriptor-is-the-contract)); clock and scheduler ([ADR 0010](decisions/0010-lifecycle-capabilities.md)) | Throwing calls, resources, additional approved effects, a closed vocabulary shared with descriptors. |
...
remaining native slice is throwing calls and owned results (catalogue B1).
ADR 0010 implements the clock and scheduler capabilities, scheduled activation,
and evaluation-time input activity. Non-recordable storage, input access in
lifecycle hooks, and external resource ownership remain B2. The vocabulary
must be closed and shared between HGL source, descriptors and the
