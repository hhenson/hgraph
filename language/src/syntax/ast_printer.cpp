...
namespace hgl::syntax
{
    namespace
    {
        ...
        class Printer
        {
          ...
          private:
            void line(int depth, std::string_view kind, SourceRange range, std::string details, std::string_view slot = {}) {
            ...
            void decl_node(int depth, SourceRange range, const ast::ModuleDecl &d) {
                ...
                if (!d.part.empty()) { details += " part " + std::string{d.part.text}; }
                line(depth, "ModuleDecl", range, details);
            }
            ...
            void type(int depth, ast::TypeId id, std::string_view slot = {}) {
                ...
                for (const ast::GenericArgument &argument : node.arguments) {
                    ...
                    if (argument.type != ast::no_node) {
                        type(depth + 2, argument.type);
                    } else if (argument.value != ast::no_node) {
                        expr(depth + 2, argument.value);
                    } else {
            ...
            void expr_node(int depth, SourceRange range, const ast::SequenceLiteral &e, std::string_view slot) {
                ...
                for (const ast::SequenceElement &element : e.elements) {
                    ...
                    const SourceRange element_range = module_.expr(element.key).range.join(module_.expr(element.value).range);
    ...
    std::string print_ast(const ast::Module &module) { return Printer{module}.run(); }
}  // namespace hgl::syntax
