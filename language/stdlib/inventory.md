...
Status: recovered historical checkpoint at hgraph `8ab0c9438`; refresh pending

This inventory made the C++ library finite at the recorded revision before any
implementation was removed. At that checkpoint the public headers contained
212 operator marker declarations with
...
The authoritative current contracts remain the headers under
`include/hgraph/lib/std/operators`. This table is recovered design evidence,
...
templates under `HGL-MIG-015` until a portable publication and reification
model is agreed.
