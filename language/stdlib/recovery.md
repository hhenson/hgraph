...
| Nine standard-library operator families | [`hgl/hgraph/std`](hgl/hgraph/std) | Restored here for design review; excluded from all builds. |
...
The operator-family files intentionally preserve open questions. A
...
Resolving a requirement should update its ledger entry first, then move the
smallest affected candidate into compiled source with parity coverage. The
design corpus should not itself be made part of the compiler acceptance suite.
