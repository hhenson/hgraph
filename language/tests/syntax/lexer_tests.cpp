...
TEST_CASE("assembled source locations retain their original files", "[source][module-parts]") {
    SourceFile file{"assembled.hgl", "one\n\ntwo\n",
                    std::vector<SourceOrigin>{{{0, 4}, "one.hgl", "one\n"}, {{5, 9}, "two.hgl", "two\n"}}};

    CHECK(file.source_path(1) == "one.hgl");
    CHECK(file.location(1) == Location{1, 2});
    CHECK(file.line_text_at(1) == "one");
    CHECK(file.source_path(4) == "one.hgl");  // zero-width diagnostic at the first part's EOF
    CHECK(file.source_path(5) == "two.hgl");
    CHECK(file.location(7) == Location{1, 3});
    CHECK(file.line_text_at(7) == "two");
}

TEST_CASE("keywords, identifiers and the placeholder", "[lexer]") {
    Lexed lexed{"part fn export abstract struct impl instantiate requires is null native _ _x in tuple"};
    REQUIRE(kinds(lexed) == std::vector<TokenKind>{TokenKind::KwPart, TokenKind::KwFn, TokenKind::KwExport, TokenKind::KwAbstract,
                                                   TokenKind::KwStruct, TokenKind::KwImpl, TokenKind::KwInstantiate,
                                                   TokenKind::KwRequires, TokenKind::KwIs, TokenKind::KwNull, TokenKind::Identifier,
                                                   TokenKind::Placeholder, TokenKind::Identifier, TokenKind::Identifier,
                                                   TokenKind::Identifier, TokenKind::EndOfFile});
    REQUIRE(lexed.result.tokens[10].text == "native");
    REQUIRE(lexed.result.tokens[12].text == "_x");
    REQUIRE(lexed.result.tokens[13].text == "in");
}
...
TEST_CASE("cpp include headers preserve delimiter form", "[lexer][native][include]") {
    ...
    REQUIRE(kinds(system) ==
            std::vector<TokenKind>{TokenKind::KwCpp, TokenKind::Identifier, TokenKind::CppHeader, TokenKind::EndOfFile});
...
TEST_CASE("cpp include requires a literal non-empty header", "[lexer][native][include]") {
    SECTION("computed header") {
        ...
        CHECK(lexed.diagnostics.diagnostics()[0].message == "expected '<header>' or '\"header\"' after 'cpp include'");
    }
...
TEST_CASE("source fragments retain every byte without coalescing trivia", "[lexer][source-accurate]") {
    ...
    const std::vector<SourceFragmentKind> fragment_kinds{
        SourceFragmentKind::Token,       SourceFragmentKind::Whitespace, SourceFragmentKind::LineComment,
        SourceFragmentKind::LineBreak,   SourceFragmentKind::LineBreak,  SourceFragmentKind::Whitespace,
        SourceFragmentKind::LineComment, SourceFragmentKind::LineBreak,  SourceFragmentKind::Token,
    };
    std::vector<SourceFragmentKind> actual_kinds;
    for (const SourceFragment &fragment : lexed.result.fragments) { actual_kinds.push_back(fragment.kind); }
...
TEST_CASE("integer and float literals", "[lexer]") {
    ...
    REQUIRE(ks == std::vector<TokenKind>{TokenKind::IntLiteral, TokenKind::FloatLiteral, TokenKind::FloatLiteral,
                                         TokenKind::FloatLiteral, TokenKind::IntLiteral, TokenKind::Dot, TokenKind::EndOfFile});
...
TEST_CASE("duration literals", "[lexer]") {
    ...
    const auto ks = kinds(lexed);
    REQUIRE(ks == std::vector<TokenKind>{TokenKind::TemporalLiteral, TokenKind::TemporalLiteral, TokenKind::TemporalLiteral,
...
TEST_CASE("a minus after a date or instant starts a new token", "[lexer]") {
    ...
    REQUIRE(kinds(lexed) == std::vector<TokenKind>{TokenKind::TemporalLiteral, TokenKind::Minus, TokenKind::TemporalLiteral,
                                                   TokenKind::TemporalLiteral, TokenKind::Minus, TokenKind::TemporalLiteral,
                                                   TokenKind::TemporalLiteral, TokenKind::Minus, TokenKind::TemporalLiteral,
                                                   TokenKind::EndOfFile});
    REQUIRE_FALSE(lexed.diagnostics.has_errors());
...
TEST_CASE("punctuation and operators", "[lexer]") {
    ...
    REQUIRE(kinds(lexed) ==
            std::vector<TokenKind>{TokenKind::LParen,      TokenKind::RParen,     TokenKind::LBrace,       TokenKind::RBrace,
                                   ...
                                   TokenKind::Comma,       TokenKind::Colon,      TokenKind::ColonColon,   TokenKind::Dot,
...
TEST_CASE("closing angle brackets are single tokens", "[lexer]") {
    ...
    REQUIRE(kinds(lexed) == std::vector<TokenKind>{TokenKind::Identifier, TokenKind::Less, TokenKind::Identifier, TokenKind::Less,
                                                   TokenKind::KwF64, TokenKind::Comma, TokenKind::KwF64, TokenKind::Greater,
                                                   TokenKind::Greater, TokenKind::EndOfFile});
}

TEST_CASE("unexpected characters are diagnosed once per code point", "[lexer]") {
    Lexed lexed{"a $ b \xC3\xA9 c"};
    REQUIRE(kinds(lexed) == std::vector<TokenKind>{TokenKind::Identifier, TokenKind::Error, TokenKind::Identifier, TokenKind::Error,
                                                   TokenKind::Identifier, TokenKind::EndOfFile});
