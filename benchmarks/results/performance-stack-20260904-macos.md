...
- date: 2026-09-04T20:00:27+00:00
...
- current-source revision: 6e47d30b393d
- current-source fingerprint: 915e4b4a8728f66f6d50b347beb46a15ec56104da5700070e80edc13dcb7b69a
...
| One source fanning out to many sinks (`scheduler_fan_out_std`) | 20000 | 0.056s +/- 0.000s | 0.056s +/- 0.001s (x1.0) |
...
| Python compute with injected GlobalState (`python_global_state_boundary`) | 20000 | 0.016s +/- 0.000s | 0.016s +/- 0.000s (x1.0) |
...
| String concatenation (`type_str_std`) | 20000 | 0.013s +/- 0.000s | 0.014s +/- 0.001s (x1.0) |
...
| One field projection - native layout (`python_owned_project_one_native`) | 20000 | 0.012s +/- 0.000s | 0.012s +/- 0.000s (x1.0) |
| One field projection - Python-owned (`python_owned_project_one_python`) | 20000 | 0.011s +/- 0.000s | 0.011s +/- 0.000s (x1.0) |
| Three field projections - native layout (`python_owned_project_several_native`) | 20000 | 0.017s +/- 0.000s | 0.016s +/- 0.000s (x1.1) |
| Three field projections - Python-owned (`python_owned_project_several_python`) | 20000 | 0.018s +/- 0.000s | 0.017s +/- 0.001s (x1.1) |
...
| Set add/remove deltas (`tss_add_remove_std`) | 5000 | 0.010s +/- 0.000s | 0.010s +/- 0.000s (x1.0) |
...
| Map only (`tsd_dense_map_std`) | 1000 | 0.071s +/- 0.002s | 0.075s +/- 0.000s (x1.0) |
| Reduce only (`tsd_dense_reduce_std`) | 1000 | 0.033s +/- 0.000s | 0.033s +/- 0.000s (x1.0) |
| String-key map and reduce (`tsd_dense_strkeys_std`) | 1000 | 0.098s +/- 0.000s | 0.109s +/- 0.001s (x0.9) |
...
| Map only (`tsd_sparse_map_std`) | 2000 | 0.022s +/- 0.001s | 0.024s +/- 0.000s (x0.9) |
| Reduce only (`tsd_sparse_reduce_std`) | 2000 | 0.011s +/- 0.001s | 0.011s +/- 0.000s (x1.1) |
| Large retained capacity with two updates per cycle (`tsd_sparse_large_capacity_std`) | 5000 | 0.903s +/- 0.002s | 0.911s +/- 0.015s (x1.0) |
...
| Key replacement - map only (`tsd_churn_map_std`) | 2000 | 0.032s +/- 0.000s | 0.033s +/- 0.000s (x1.0) |
| Key replacement - reduce only (`tsd_churn_reduce_std`) | 2000 | 0.008s +/- 0.000s | 0.008s +/- 0.000s (x1.0) |
| Monotonic key growth across capacity boundaries (`tsd_capacity_growth_std`) | 1000 | 0.036s +/- 0.000s | 0.034s +/- 0.000s (x1.0) |
| Full clear followed by repopulation (`tsd_clear_repopulate_std`) | 200 | 0.423s +/- 0.002s | 0.430s +/- 0.002s (x1.0) |
| Remove and later recreate the same keys (`tsd_key_reactivation_std`) | 2000 | 0.022s +/- 0.000s | 0.023s +/- 0.000s (x1.0) |
| Two-input map with union membership (`tsd_two_input_union_std`) | 2000 | 0.033s +/- 0.000s | 0.034s +/- 0.000s (x1.0) |
| Two-input map with intersection membership (`tsd_two_input_intersection_std`) | 2000 | 0.019s +/- 0.000s | 0.019s +/- 0.000s (x1.0) |
...
| Empty/singleton/two-value TSD without zero (`reduce_tsd_without_zero_std`) | 20000 | 0.022s +/- 0.000s | 0.023s +/- 0.000s (x1.0) |
...
| Request/reply service - native implementation (`service_request_reply_std`) | 20000 | 0.026s +/- 0.000s | 0.027s +/- 0.000s (x1.0) |
...
| Subscription service - Python implementation (`service_subscription_py`) | 20000 | 0.044s +/- 0.000s | 0.043s +/- 0.001s (x1.0) |
...
| Multiplexed service adaptor - native implementation (`service_adaptor_std`) | 20000 | 0.029s +/- 0.001s | 0.029s +/- 0.001s (x1.0) |
| Multiplexed service adaptor - Python implementation (`service_adaptor_py`) | 20000 | 0.028s +/- 0.000s | 0.028s +/- 0.001s (x1.0) |
...
| TSD getitem with a ticking key (`audit_tsd_getitem_std`) | 20000 | 0.033s +/- 0.001s | 0.033s +/- 0.000s (x1.0) |
...
| CompoundScalar to_json/from_json round trip (`audit_json_roundtrip_std`) | 10000 | 0.013s +/- 0.000s | 0.016s +/- 0.000s (x0.8) |
...
| Stream take/drop/step counters (`audit_take_drop_std`) | 20000 | 0.014s +/- 0.000s | 0.014s +/- 0.000s (x1.0) |
| Running mean accumulator (`audit_running_mean_std`) | 50000 | 0.023s +/- 0.000s | 0.024s +/- 0.001s (x1.0) |
