...
def capture_output_to_global_state(path: str, ts: REF[TIME_SERIES_TYPE], _global_state: GlobalState = None):
...
def capture_output_to_global_state_start(path: str, ts: REF[TIME_SERIES_TYPE], _global_state: GlobalState = None):
...
def capture_output_to_global_state_stop(path: str, _global_state: GlobalState = None):
    """Clean up references"""
    _global_state.pop(output_key(path))


@sink_node(active=tuple(), valid=tuple())
def capture_output_node_to_global_state(path: str, ts: TIME_SERIES_TYPE, _global_state: GlobalState = None):
...
def capture_output_node_to_global_state_start(path: str, ts: TIME_SERIES_TYPE, _global_state: GlobalState = None):
    _global_state[path] = ts.output.owning_node


@capture_output_node_to_global_state.stop
def capture_output_node_to_global_state_stop(path: str, _global_state: GlobalState = None):
    del _global_state[path]


@sink_node
def write_subscription_key(path: str, key: TS[SCALAR], _state: STATE = None, _global_state: GlobalState = None):
    """
    ...
    """
    svc_node_in = _global_state.get(f"{path}/subs")
...
def write_subscription_key_start(path: str, _state: STATE, _global_state: GlobalState = None):
...
def write_subscription_key_stop(path: str, _state: STATE, _global_state: GlobalState = None):
    if (key := _state.previous_key) is not None:
        (s := _state.tracker[key]).discard(_state.subscription_id)
        if not s:
            del _state.tracker[key]
            if subs_in := _global_state.get(f"{path}/subs"):
                subs_in.apply_value({
                    Removed(key),
                })
...
def write_service_request_start(path: str, _state: STATE, _global_state: GlobalState = None):
    global_state = _global_state
...
def write_service_request_stop(
    request: TIME_SERIES_TYPE, path: str, _state: STATE, _global_state: GlobalState = None
):
    if request.valid:
        global_state = _global_state
...
@sink_node
def write_service_replies(path: str, response: TIME_SERIES_TYPE, _global_state: GlobalState = None):
    """
    Updates TSDs attached to the path with the data provided.
    """
    svc_node_in = _global_state.get(f"{path}/replies_fb")
...
@generator
def request_id(hash: int, _state: STATE = None, _global_state: GlobalState = None) -> TS[int]:
    global_state = _global_state
    i = global_state.get("request_id", id(_state)) + 1
    global_state.request_id = i
    yield timedelta(), i
...
def write_adaptor_request(
    ...
    from_graph_node = _global_state.get(f"{path}/{arg}")
...
def write_adaptor_request_stop(
    path: str, arg: str, requestor_id: TS[int], _state: STATE, _global_state: GlobalState = None
):
    if from_graph_node := _global_state.get(f"{path}/{arg}"):
...
@sink_node
def write_service_requests(path: str, request: TIME_SERIES_TYPE, _global_state: GlobalState = None):
    """
    Updates TSDs attached to the path with the data provided.
...
def get_shared_reference_output(
    path: str, strict: bool = True, node: NODE = None, _state: STATE = None, _global_state: GlobalState = None
...
def get_shared_reference_output_stop(
    path: str, node: NODE = None, _state: STATE = None, _global_state: GlobalState = None
):
    subscriber = _global_state.get(f"{path}_subscriber")
