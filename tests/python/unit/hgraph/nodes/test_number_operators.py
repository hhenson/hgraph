...
from hgraph import add_, sub_, div_, exception_time_series, TS, graph, mod_, divmod_, pow_, eq_, const, DivideByZero
...
def test_div_numbers(lhs, rhs, divide_by_zero, expected):
    ...
    assert math.isnan(output) if math.isnan(expected) else expected
