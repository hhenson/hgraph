...
`\uNNNN`, and the rest `\UNNNNNNNN`. Printability follows the Unicode version
the implementation records; a character whose category differs between
Unicode versions may render differently under another Python.
...
- **OP-9** `str_` of a container renders each string as Python's `repr`,
  escaping every character `str.isprintable` rejects.
...
2. **The order of a map's text.** A map has no order and every value has one
   ...
   of a removed key. Equal maps can therefore render differently in both.
   Raised as a question for the owner.
