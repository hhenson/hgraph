...
The downcast allows a value to be re-typed to a derived type. It applies to
``CompoundScalar`` hierarchies: a ``TS`` of some base type is narrowed to a
``TS`` of a class derived from it, and the narrowing is checked at runtime.
The target type is supplied as a type parameter:

.. testcode::

    from dataclasses import dataclass
    from hgraph import graph, TS, CompoundScalar, downcast_
    from hgraph.test import eval_node
    ...
    @graph
    def as_bond(ts: TS[Instrument]) -> TS[Bond]:
        return downcast_[TS[Bond]](ts)

    assert eval_node(as_bond, [Bond(symbol="UST10Y", coupon=0.04)]) == [
        Bond(symbol="UST10Y", coupon=0.04)
    ]

The input is declared as ``TS[Instrument]``, so as far as the type system is
concerned a ``Bond`` cannot be read out of it — the ``downcast_`` is what makes
that legal. If a value arrives that is *not* actually a ``Bond``, the downcast
fails at runtime rather than silently mis-typing the value.

``downcast_ref`` performs the corresponding narrowing on a reference without
the runtime check, and it takes the target type as an ordinary argument:
``downcast_ref(Bond, ts)``.

.. note:: ``downcast_`` narrows within a ``CompoundScalar`` hierarchy. It is
          not a general scalar conversion — narrowing ``TS[date]`` to
          ``TS[datetime]``, which the Python-first implementation on
          ``release/0.5`` allowed, is not supported. Use ``cast_`` for
          conversions between unrelated scalar types.
...
nor will there be nodes for the mathematical operations. Fewer nodes sounds cheaper, but it is usually not: the
runtime engine is C++, so the graph form evaluates the arithmetic natively, while the ``compute_node`` form pays a
Python call — and the boxing of its inputs and result — on every tick. Prefer the graph form and drop into a
``compute_node`` when the logic genuinely cannot be expressed as a graph.
...
Note the use of ``v.value`` rather than ``v.delta_value``. The node fires when *either* the value
or the key ticks, and in the cycle where only the key ticks the selected value has not been modified —
so its ``delta_value`` is empty. ``value`` is the property that always holds the current state.
