...
It is served over the Perspective adaptor, so install the extras first:

.. code-block:: bash

    pip install "hgraph[web,perspective]"
...
Do not remove the ord and id columns as they are required for the inspector to work correctly. You can move them to the
