...
The ``hgraph`` distribution provides the public Python package on top of the
C++ runtime. Version 0.8.0 replaces the Python-first implementation; that
implementation remains on the ``release/0.5`` maintenance line and is used as
a pinned compatibility reference in a separate environment.
...
API is the compatibility commitment for the ``0.8`` release line. The native
...
intentionally not exposed by C++-first ``hgraph``. Use the public service, adaptor, and
...
record/replay machinery. C++-first hgraph deliberately does not replicate the legacy
...
C++-first hgraph extension for callers that already hold a PyArrow table; both feed the
