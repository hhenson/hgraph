...
**Status:** Initial Draft — being reconciled against the 0.8 C++ runtime

---

## Document Purpose

This specification is a language-neutral definition of the HGraph programming model and runtime system. It is designed to enable implementation of compatible HGraph runtimes in any programming language while maintaining semantic equivalence with the reference implementation.

The reference implementation is the C++ runtime in this repository. The `hgraph` Python package is an authoring surface over that runtime, not a second implementation of it.
...
The reference implementation is the C++ runtime in this repository:
- **Public headers**: `include/hgraph/` — `types/value/`, `types/metadata/`, `types/time_series/`, `runtime/`
- **Runtime**: `src/hgraph/`
- **Python authoring surface**: `python/hgraph/`
- **Tests**: `tests/cpp/` (Catch2) and `python/tests/` (pytest)

Its internal design — memory layout, plans and ops tables, schema representation, the wiring boundary — is recorded in the [Developer Guide](../developer_guide/index.rst), which is authoritative for implementation questions this specification deliberately leaves open.
...
These documents were originally written against the Python-first implementation, which is maintained on the `release/0.5` branch. Behaviour that intentionally differs between the two is listed in the [Python parity matrix](../developer_guide/parity_matrix.rst) — that document, not this one, is the authoritative record of accepted deviations.
...
*This specification is derived from the HGraph reference implementation and existing design documents. Sections not yet reconciled against the 0.8 C++ runtime should be treated as provisional rather than normative.*
