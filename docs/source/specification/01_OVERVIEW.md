...
- A declarative surface for defining reactive computations — a Python DSL and a native C++ API, both describing the same graphs
...
The reference implementation is the C++ runtime; the Python package is an
authoring surface over it, not a parallel implementation.

| Component | C++ (reference implementation) | Python authoring surface |
|-----------|-------------------------------|--------------------------|
| Values and value views | `include/hgraph/types/value/` | — |
| Schemas, bindings, registries | `include/hgraph/types/metadata/` | `python/hgraph/_types.py` |
| Time-series types | `include/hgraph/types/time_series/` | `python/hgraph/_types.py` |
| Runtime and execution | `include/hgraph/runtime/`, `src/hgraph/runtime/` | — |
| Operator library | `include/hgraph/lib/std/operators/` | resolved lazily via `hgraph.__getattr__` |
| Wiring | `src/hgraph/` | `python/hgraph/_wiring/` |
| Test harness | `include/hgraph/lib/testing/` | `python/hgraph/test/` |

For how the C++ side is structured internally — plans, ops tables, interning,
the linking strategy — see the [Developer Guide](../developer_guide/index.rst).
