...
Record and replay are driven by a *scope* placed around a `@component`, plus a
configured store. The scope decides what the component does with its recorded
inputs and outputs; the component's name qualifies the recorded keys.

**Example 1: Record a component's inputs and output**

```python
import hgraph as hg
from hgraph import graph, component, TS
from hgraph.test import eval_node
...
assert eval_node(recording, [1, None, 3], [10, 20, None]) == [11, 21, 23]
assert hg.frame_store_contains("calc.lhs")
```

**Example 2: Replay those recordings over live inputs**
...
# The recordings win over the live inputs.
assert eval_node(replaying, [100, 100, 100], [100, 100, 100]) == [11, 21, 23]
```

`RecordReplayEnum` also provides `COMPARE`, which checks a live run against the
recordings and reports through `comparison_summary`, and `RECOVER`, which seeds
from the recordings at start and then follows live inputs.
