...
```python
from hgraph import graph, TS, TSD, TSS, map_, add_

@graph
def map_test(keys: TSS[str], ts1: TSD[str, TS[int]], ts2: TSD[str, TS[int]]) -> TSD[str, TS[int]]:
    return map_(add_, ts1, ts2, __keys__=keys)
```

The mapped function is the public `add_` operator; `map_` resolves the overload
for the element type it is instantiated with. There is no need to name a
